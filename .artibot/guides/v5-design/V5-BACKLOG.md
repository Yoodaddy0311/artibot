...
| wave11-hygiene-bundle — 창 보고 소유 밖 5건 묶음 | todo | Wave 11 창들이 보고만 하고 소유 밖이라 손대지 않은 것들: `probe-hook-keys.test.js:32` 픽스처 목록 하드코딩 → `readdir` 열거로(새 host-payload 픽스처가 게이트를 타게) · HG-07 nonce · `counts` null-proto · reporter `scope` 필드 · `sleepSync` 사본 3 + `renameWithRetry` 사본 1 을 한 곳으로. existence-audit `:369` null-proto 는 `sh29-command-hook-carriers` 가 흡수하자는 제안이 있다 — plan 시 결정 | (랜딩 게이트 신뢰) |
| session-store-hygiene — 테스트 잔재 격리 | todo | `engine-state.test.js` 가 `runtime/autopilot/test-engine-state-*.json` 을 남긴다(2회 실행 후 32개, 기존재·gitignored) → cwd 격리 · `cost-predictor.js:122` `listSessions` 의 `slice(-50)` 꼬리가 전부 그 잔재라 `predictCost` 가 항상 n=0(usage 이벤트 0/9,929) → `test-*` 제외. **삭제는 O6(격리 착지) 뒤** | Observe(비용 예측 분모) |
| team-config-semantics — team 스위치 의미 단일화 | todo | `config-schema.js:57-67` 의 `team.properties` 가 7키만 선언해 `followWorkflowPlan`·`autoApply`·`autoApplyTriggers`·`api` 4키가 미선언(non-strict 라 통과) → 선언 추가 · `create-artibot-agent.js:319` `teamEnabled: Boolean(config?.team?.enabled)` 와 `self-benchmark.js:159` `Boolean(config?.team?.autoApply)` 가 텔레메트리 봉투에서 **세 번째 의미**를 만든다 → 둘 다 `workflow-plan.js#isTeamEnabled` 경유로(OD3 정합) | Canary(team 자동 트리거) |
| OB-07 후속 — 분자 호출률 측정 수단 | todo | 배치 9 로 판독기는 생겼으나 **호출률 자체를 재는 수단이 여전히 0**. 후보 B′ = PostToolUse exit-code 관찰자 · 원장 tail 10MB 트리거. **Wave 13** 로 미룬다(Wave 12 는 C4 producer·verdict adapter `warn` 선행) | Observe ③ 분자 |
