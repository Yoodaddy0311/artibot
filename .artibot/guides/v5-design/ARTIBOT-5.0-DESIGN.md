...
- `lib/recovery/{failure-classifier,recovery-controller,plan-repair}.js` — engine 위의 분류기. 배선 지점은 **VERIFY 결과 기록 시점 `engine-state.js#recordPhaseResult`**(CA-03 이 `pendingPhase` 를 덮을 자리와 같다)이고, 거기서 `nextPhase` 를 `decide()` 결과로 바꾼다(기존 `onFailure.{retryLimit,escalateTo}` 지시 필드의 실제 소비자). 현행 **고정 전이 = 엔진 `nextPhase` IMPROVE + 드라이버 산문**(3회 재시도 → PAUSED). `unknown` 은 사람으로 승격, `framing→Ultraplan` 은 제안만.
...
| **GA** | 확대 | 저위험 외로 확대, 4티어 전면, split 자동 진입(WP02 선행). **GA 비의존**(§9·§58 — GA 를 여기에 의존시키지 않는다): online RL · 재귀 에이전트 생성 · 무제한 debate · provider 은닉 캐시 해킹 · `cache_transfer`. **오너 결정 G1(2026-09-14)**: v5.0 은 GA-02 기전만, GA-01·GA-03 은 v5.1 | — | 파괴·배포·외부쓰기·제품결정 사람 게이트 **영구** |
...
| §9·§58 | Cross-model KV cache reuse 는 Watch only, GA 를 online RL·재귀 에이전트 생성·무제한 debate·provider 은닉 캐시 해킹에 의존시키지 말 것 | Hardening §36 은 재귀 깊이 상한 하나뿐 | **확장 채택**(재분류) — GA 비의존 목록 5종을 §4 GA 행의 제외 조건으로 명문화, `model_capabilities.cache_transfer{supported:false, mode:'none'}` 는 **CA-02 작동기 착수 시 예약**한다 — **v5.0 에서는 미예약**이다(`plugins/` 하위 `cache_transfer`·`cacheTransfer` **0건**, 리포 전역 hit 는 이 설계문서와 `MODEL-SWITCHING-SCORECARD.md:614` 제안문뿐 — 스키마·코드 0, 실측 2026-09-21). 추가이므로 "일치" 아님 |
...
|---|---|---|---|
| W11-Q1 | OB-17 D1 — 스폰 단위 receipt 의 incumbent(`models.current`) 원천 | **K1 리더 세션 모델.** `route-receipt.schema.json` "세션 스코프" 정의·`model-switcher.js` 와 일치, 스키마 무변경. `session-start.js` 가 payload `model` 키를 읽지 않으므로 호스트 프로브가 ob17 줄기 첫 단계 | 프로브에서 호스트 payload 에 `model` 키가 없으면 K2(같은 세션 직전 스폰의 selected) 재상정. **기전 확정(11:1x, ob17 창 B0 프로브 2.1.272)**: 호스트 payload 에 `model` 키 0 → 전사 tail 의 마지막 assistant `message.model` 을 카탈로그 티어로 사상, 의미는 K1 불변 |
