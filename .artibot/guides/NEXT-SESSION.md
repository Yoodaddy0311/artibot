...
- [x] **릴리스 순서 준수 — v4.63.0 (2026-09-17)**: `ci/release-4.63.0` 푸시 → CI green → master ff → 태그 (gotcha 103, `-u` 금지). 실행 기록 — 릴리스 커밋 SHA: `(단계 B 에서 기입)` · `ci/**` 푸시 시각: `(단계 B)` · CI green run id·시각: `(단계 B)` · master ff SHA·시각: `(단계 B)` · 태그 `v4.63.0` 푸시 시각: `(단계 B)`.
