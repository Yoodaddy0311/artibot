...
**다음 할 일**: ① F07 수리 + config 등재 커밋 착지 ② `sync:local` → 호스트 재시작 → 판독기 4종(`verify-rate`·`session-coverage`·`nl-activation-report`·`outcome-census`) 라이브 실행 + 기준 7 프로브 ③ 4.63.0 판단 — **체크리스트 항목 1·3 은 2026-09-15 17:49 완료**(구 원장 2파일 + 리더 백업 삭제, ADR-011 행 done), **항목 2(plugin-local 28행)는 이월**(`migrate-ledger-adr011.mjs` 가 concat 전용이라 재사용 불가, 28행은 `idempotency_key` 0 · 라이브 미포함이라 보존) ④ 정리 잔여(locked worktree 6 + verify 창, junction 껍데기 2, `ob17-models-current` 강제 삭제 `-D` 는 오너 승인 O1 로 확정 — 실행만 남음) ⑤ 리더 todo 33건을 Wave 12 로 분류.
...
- [ ] **패키징 회귀 재확인**: 4.63.0 빌드 후 신규 머신(또는 sandbox HOME) 설치 스모크로 `schemas/ledger-events.allowlist.json` 실재 재확인 — `df85f702` 가 이미 `install-files-smoke.test.js` 에 회귀 가드를 넣었으므로 `npm run ci` 통과가 곧 이 항목의 충족 증거.
