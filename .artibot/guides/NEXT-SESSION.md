...
- [ ] **구 원장 파일 삭제**(ADR-011 절차 ⑤, 보존 기간 만료 = 다음 릴리스): `.artibot/runtime/ledger.jsonl.pre-adr011`(204,406B, 2026-09-11 16:36) · `.git/artibot/ledger.jsonl.live-premerge-20260914`(442,576B, 2026-09-14 12:29). 둘 다 실측 확인됨(2026-09-14). 삭제 전 738행 이관 결과(`a9f35e0d…`)가 여전히 유효한지 재검증 1회 권장(diff 없는 파일이므로 낮은 리스크).
- [ ] `plugins/artibot/.artibot/runtime/ledger.jsonl`(28행, 프로브 실행 잔재로 추정 — **미확인**, 프로젝트 원장이 아니라 이 세션 중 로컬 검증 부산물인지 git blame/최초 커밋으로 먼저 확인 후 처분 판단)
- [ ] `V5-BACKLOG.md:173` ADR-011 행을 "이관 done / 삭제 done" 으로 갱신(현재 "이관 done / 삭제 대기")
