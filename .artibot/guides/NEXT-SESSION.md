# NEXT-SESSION — 크로스머신 핸드오프 (2026-09-14 KST 갱신, AsusHeechangLee 머신, master = 34940d2b, Wave 8 8/8 착지 + worktree·브랜치 정리 완료, v4.61.0 미릴리스, 호스트 로드 설치본 4.59.0 · sync:local 4.60.0)
...
**다음 할 일**: ① **v4.61.0 릴리스** — `ci/release-4.61.0` 경유 랜딩, 체크리스트에 **`runtime.artifactLifecycle.enabled` 를 `false` 로**(결정 ①, config 1줄 + 핀 테스트 2) · **이관 실행 뒤 `.pre-adr011` 삭제(이관 미실행이면 no-op)** · README 카운트 28/72 확인 · 산문 동기화(`sync:readme:claims` 를 로컬에서 먼저, gotcha 74) 포함. **`.pre-adr011` 정정**: 지금 파일 0건은 "삭제 완료" 가 아니라 **이관을 안 돌려서 생긴 적이 없는 것**이다(V5-BACKLOG:165 in-progress, migrate 스크립트 실행 흔적 0). ② 오너가 `claude plugin update` 로 설치본 갱신 — **설치본 라벨 정정**: `installed_plugins.json` 라벨은 4.59.0 인데 cache 내부 `package.json` 은 4.60.0 이다(라벨 갱신 주체 미확인). 어느 복사본에도 `intent-observe-pre` 가 없으므로 **4.61.0 갱신 시점이 그 훅의 첫 라이브 발화**다 ③ 갱신 후 **Observe ③축 재집계**(`review.completed` 분모 — 호출부가 배포되기 전까지는 구조적으로 0) + 실 원장 이관(`scripts/ledger/migrate-ledger-adr011.mjs`, 선존재 exit 3) ④ **Wave 9**(플레이스홀더 — 리더가 plan 확정 후 base SHA·줄기 목록을 이 절에 채운다). 후보: `guard-command-position`(브리프 + leader-addendum 완비) · DR02 재개 주입(SH-21 잔여) · dev-verify-gate 배선(OB-07/CA-13) · SH-02 plan/review/outcome.md 핸들러 실행 · `resolveDispatch` 완료줄기 선제외(부채, 25% 상한 안). 브리프마다 "리더 통합 단계" 절(gotcha 90·91)을 필수로 넣는다.
