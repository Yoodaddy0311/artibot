...
**라이브 재계수 (2026-09-21 09:2x~09:4x KST, 세션 artibot-35/391b5a, 설치본 4.64.0 = package.json mtime 2026-09-17T05:19:25Z, 호스트 재시작 2026-09-21T00:10:19Z)** — 팀 investigator 3 + auditor 3(fable), 리더 재확인 완료:
...
**gotcha 신규**: (116) **`git worktree remove` 뒤 junction 껍데기** — gotcha 113 이 이번 웨이브에서 재확인됐다. 제거 정본은 여전히 `worktree-setup.mjs --teardown`, 재귀 삭제 금지. (117) **창이 한 도구 호출에 수 시간 busy 로 앉아 있을 수 있다** — 그동안 transcript 에 기록이 남지 않고 크로스세션 메시지는 큐에만 쌓인다. 판정은 **transcript mtime + assistant 턴 수**로 하고 `ListAgents` 의 busy/idle 은 근거로 쓰지 마라. 복구 = 오너 Esc 또는 다른 창으로 줄기 재배정(이번 verify 창이 그 사례). (118) **브리프 allowlist 를 글롭으로 쓰면 plan.json 과 어긋난다** — 브리프가 `tests/hooks/auto-team-trigger*.test.js` 같은 글롭인데 `plan.json#affectedPaths` 는 파일 단위라 창이 plan 밖 파일을 커밋했다(f04b 13:3x). 규칙 = 브리프도 plan.json 과 **정확히 같은 파일 목록**으로 쓰고, 창은 **커밋 전마다 `git diff --name-only` ⊆ affectedPaths** 를 대조한다. (119) **리더 지시 교차 금지** — 한 라운드에 한 지시만 내고, 이전 지시를 물릴 때는 **철회를 명시**하라. (120) **이 리포의 전체 게이트는 `npm run ci` 다** — prebuild·build 단계가 없으므로 "prebuild 통과" 를 근거로 쓰면 안 된다. (121) **Windows `renameSync` EPERM 은 확률적이다** — 목적지 파일이 이미 있고 누가 핸들을 쥐고 있을 때 발생하며 부하 시 0.83%. 수리 = `renameWithRetry` 공용화(현재 사본 4곳: `session-store.js`·`lock.js`·`file-lock.js`·`split-brief.js`). (122) **MSYS `sed -i` 는 CRLF 문서의 CR 을 전부 제거한다** — 이 리포의 `.md` 정본은 대부분 CRLF 인데 `sed -i` 로 치환하면 파일 전체가 LF 로 바뀐다. `core.autocrlf=true` 라 **`git diff` 에는 안 보이므로** 눈치채지 못한 채 커밋된다. 문서 치환은 **Edit 도구**나 스크래치패드 + `awk '{printf "%s\r\n", $0}'` 로 하고, 편집 뒤 `tr -cd '\r' | wc -c` 와 `tr -cd '\n' | wc -c` 가 같은지 확인하라(2026-09-15 리더 docs 패스에서 3파일 발생, 즉시 복원).

**Wave 12 정찰 초안 8건(2026-09-15 18:3x~22:4x KST, 리더 artibot-28, 유휴 팀원 investigator×5 + planner, 코드 0)**: `.artibot/split/` 아래 8개 줄기 디렉터리의 `brief-draft.md` — `ca03-nextphase-decide`(100줄) · `nl-activation-numerator-writer`(98) · `sh29-command-hook-carriers`(94) · `route-bound-canonical-model`(86) · `report-generator-recovery-journal`(77) · `team-config-semantics`(52) · `wave11-hygiene-bundle`(49) · `session-store-hygiene`(44). **정찰이 뒤집은 리더 전제 3건**: ① CA-03 의 verdict adapter `warn` 부재는 **기능 선행이 아니다** — `recovery-record` 가 PASS 를 null 로 접어 분류하는 것이고, `ask_human` 쏠림의 진짜 원인은 드라이버가 `state.verifyResult` 를 쓰지 않는 것이다. action→phase 표는 코드에 없어 줄기가 신설한다. ② `route.bound` 의 `selected_model` 결손 **244/249 = 98%**(13:42Z)의 원인은 카탈로그 매처가 아니라 `subagent-handler.js#checkModelPolicy` 가 팀원 이름(`split-artibot-*`)을 정책 키로 풀지 못하는 것이다 — 수리는 bind 측 `subagent_type` 폴백(allowlist 무접촉)이고 ob17·O2 영향 0. ③ SH-03 은 분자만 없는 게 아니라 **예측 측 `command_activation` 도 런타임 미생산**이다. **O8 초안** = (a1) `hook.fired` 신설(디스패치 1회 = 1행, +770행/일 ≈ 3×) · command 는 `intent.detected.fields.command` 확장. **리더 결정 대기 10건**: ca03 4(repair→EXECUTE · PAUSED 표현 · divergent 갱신 · warn 어휘) · sh29 3(`:369` 흡수 · command 직렬 · inventory 철자) · nl 3(힌트 축 포함 · workflow→`/team` · recorder 위치) — nl 브리프의 "93 = it 수" 는 **REFUTED**, 실제 44. **교집합 4쌍(inspector 재계산)** — sh29 ↔ nl `runtime-prompt.js` · sh29 ↔ hygiene `existence-audit.js`(+ test) · ca03 ↔ session-store `engine-state.test.js` · ca03 ↔ report-generator `commands/autopilot.md`(조건부). 정찰이 주장한 team-config ↔ ca03 `config-schema.js` 는 **불성립**(ca03 이 스키마를 선언하지 않는다). plan 때 이 4쌍을 `conflictGroups` 로 묶는다. 원장 실측 오늘 **1,377행**(~340~380행/일).

**다음 할 일**: ① F07 수리 + config 등재 커밋 착지 ② `sync:local` → 호스트 재시작 → 판독기 4종(`verify-rate`·`session-coverage`·`nl-activation-report`·`outcome-census`) 라이브 실행 + 기준 7 프로브 ③ 4.63.0 판단 — **체크리스트 항목 1·3 은 2026-09-15 17:49 완료**(구 원장 2파일 + 리더 백업 삭제, ADR-011 행 done), **항목 4(패키징 회귀)도 완료**(`c88e699d` CI Validate 4종 green + `install-files-smoke` 14 passed/1 skipped) → **체크리스트 3/6**, **항목 2(plugin-local 28행)는 이월**(`migrate-ledger-adr011.mjs` 가 concat 전용이라 재사용 불가, 28행은 `idempotency_key` 0 · 라이브 미포함이라 보존). 잔여 3 = 28행 이월 · 릴리스 순서 준수 · Observe ③④ 재집계 ④ 정리 잔여(locked worktree 6 + verify 창, junction 껍데기 2, `ob17-models-current` 강제 삭제 `-D` 는 오너 승인 O1 로 확정 — 실행만 남음) ⑤ Wave 12 plan — 정찰 초안 8건 + 오너 결정 O3/O5/O7/O8(초안 있음) + 리더 결정 10건을 묶어 처리 → 8줄기 편성(부채 3 = 25% 경계, `team-config-semantics` + `wave11-hygiene-bundle` 병합 검토).

**4.63.0 릴리스 완료 2026-09-17** — master `e92092e5`(= `b7ac9a55` release 커밋 + badge-sync 자동 착지), 태그 `v4.63.0`, 설치본 `sync:local` 완료(`release-check.js` exit 0). **호스트 재시작 필요**(Observe ③④ 라이브 분모 t0 확보), 다음은 **Wave 12 `/split plan`**. 자세한 실행 기록은 아래 §"4.63.0 릴리스 체크리스트" 항목 5.

**오너 결정 기록**: O8 `hook.fired` 어휘 = **a1(디스패치 1회 = 1행, 핸들러 배열 fold)** — 오너 결정 2026-09-17. (O3/O5/O7 은 초안 권고대로 PROCEED, `.artibot/split/plan-wave12-draft.json#_ownerDecisions` 가 로컬 정본.)

## Wave 11 준비 + 리더 정리 (2026-09-15 09:0x~10:0x KST, 세션 artibot-28/89ada2, master = f4555c1c, 코드 변경 0)

**한 줄**: Wave 10 잔여 정리(빈 디렉터리·로컬 브랜치·unlocked worktree)를 끝내고, Observe 판독기 3종을 **처음으로 라이브 원장에 돌려 수치를 확보**했으며, Wave 11 plan 초안(8줄기 + 롤링 1)을 세웠고, 오너 결정 4건은 같은 세션 10:0x 에 전부 권장안으로 확정됐다. 이 세션은 정리·계측·기획만 했고 **코드 변경 0**이다.

**정리 완료(devops 실측)**: 미등록 빈 디렉터리 4개 `rmdir`(`nl-activation-report` · `routing-single-decision` · `scorecard-absent-contract` · `split-ops-remediation`) · 로컬 브랜치 `-d` 6개(위 4 + `autopilot-phase-transition` a88eb5e7 · `routebench-scenarios-live` 9ae3b89d — 전부 origin/master merged, ahead 0) · unlocked worktree 2개 `git worktree remove` + `prune`. 사후 실측: `git worktree list` 9행(master + locked 8) · 브랜치 `worktree-split-artibot-*` 8개 · `.claude/worktrees/` 8개 · HEAD ac721255 불변(정리 시점 09:4x, docs 커밋 f4555c1c 는 그 뒤 착지). run.json `cleanupPending.worktreeDirs` 4개는 전부 처리됐다.

**gotcha 신규**: (113) **`git worktree remove` 는 `plugins/artibot/node_modules` junction 을 남긴다** — exit 0 이어도 디스크에 `plugins/artibot/node_modules`(→ 라이브 `plugins/artibot/node_modules` 를 가리키는 NTFS junction, `scripts/split/worktree-setup.mjs:208` `fs.symlinkSync(source, target, 'junction')`) 껍데기가 남아 뒤따르는 `rmdir` 스윕이 `Directory not empty` 로 실패한다. `find -type f` 는 링크를 세지 않으므로 "빈 트리" 로 오판한다(리더 실수). **정본 제거 수단 = `node plugins/artibot/scripts/split/worktree-setup.mjs <worktreePath> --teardown`** — reparse point 만 제거하고 링크 슬롯에 실디렉터리가 있으면 거부한다(같은 파일 24~27행). 이번에는 옵션 없는 MSYS `rm <link>` 로 끊었고 타깃 무손상이었으나(상위 107 엔트리 · 파일 2,181 · vitest 실행 OK), 같은 파일 헤더 11~12행이 기록한 "junction 을 `rm -rf` 로 지우면 부모로 따라 들어간다 — 부모 957 엔트리가 한 키스트로크 앞이었다" 사고 때문에 **재귀 삭제·`Remove-Item -Recurse` 금지**. 남은 locked 8개 전부 같은 링크를 갖고 있다(리더 실측) → 정리 순서는 `unlock` → `--teardown` → `git worktree remove` → `prune` → `branch -d`(단 `ob17-models-current` 만 `-D` — 빈 커밋 a73eab24 는 의도된 미착지라 merged 판정이 안 선다). (114) **"locked = 창 생존" 은 성립하지 않는다** — 잠금 사유에 적힌 pid 8개가 전부 사망 상태였고(investigator `Get-Process`), WindowsApps `claude.exe` 10개는 Claude Desktop 앱 1개(main 45992)의 자식 프로세스이지 창이 아니다. 반면 `ListAgents` 에는 Wave 10 줄기명을 단 Remote Control 유휴 피어 8개가 idle 로 잡혔다(리더 09:1x) → 창은 데스크톱 앱 내부 세션으로 살아 있을 가능성이 높으나 **`EnterWorktree` 가능 여부는 dispatch 전 미확인**. `[split:reuse]` 성립 여부는 pid·lock 이 아니라 **SendMessage 응답으로 판정**할 것. (115) **docs 커밋도 landing-flow 게이트 대상** — `git push origin master` 직행은 pre-push 훅이 차단한다(f4555c1c 실측). 모든 master 착지는 ci/** 푸시 → 4체크 green → `--ff-only` → push 순서. 리더가 팀원에게 커밋을 지시할 때 처음부터 이 경로를 넣어라.
...
**Wave 11 plan 초안**: `.artibot/split/plan-wave11-draft.json`(gitignore). 8줄기 + 롤링 1(`outcome-md-emitter`), 부채 1/8 = 12.5%(`hg09`), `conflictGroups` 3(ob26↔f04b config · sh29↔outcome 는 dispatch-table + README 같은 줄 · sh29↔ob17 은 allowlist 조건부), 창 배정 8↔8. 정찰 초안에 남아 있는 `dispatch-base-forkpoint` 부채 언급은 **낡았다**(096d5897 로 이미 착지). **오너 결정 4건 확정(2026-09-15 10:0x, 전부 권장안)**: W11-Q1 OB-17 D1 incumbent 원천 = **K1 리더 세션 모델** · W11-Q2 OB-17 D2 = **H1 `residency-unknown` 별 코드**(H3 스키마 nullable 보류) · W11-Q3 SH-29 E5 = **(a) skill 만 지금**(command 는 Wave 12 `intent.detected`, hook 은 Wave 12 신규 이벤트) · W11-Q4 outcome 완료 선언 트리거 = **(a) SessionEnd 유도 판정** — 정본 기록은 `ARTIBOT-5.0-DESIGN.md` 부록 0-2 후속(4)(1077~1097행), plan 초안 `_ownerDecisions` 동일. 정본이 이미 답한 12항목은 질문에서 제외했다(OB-26 = 7 은 §5 D12, SH-06 D1~D3 은 §5 C8·§3.4 등).
...
**다음 할 일**: ① ~~오너 결정 4건 회신~~(확정) → plan 초안의 `parentSession`·`createdAt` 치환 → `/split plan` → dispatch(창 8개에서 `[split:reuse]` 응답 확인) ② ~~V5-BACKLOG 변경분 + 이 NEXT-SESSION 절 커밋(docs)~~ → f4555c1c 착지. 2차 docs(DESIGN 부록 후속(4) + 이 절 갱신) 착지는 별도 커밋 ③ 4.63.0 은 위 체크리스트 미충족 3건 해소 뒤.
...
**gotcha 신규**: (99) **`indexArtifacts({kind:'adr'})` 를 이 리포에서 부르지 마라** — `.artibot/adr/INDEX.md` 수기 정본(B2 재번호 표)이 자동 표로 덮이고 ADR-011 이 legacy 로 찍힌다. `git checkout` 으로 복원했다. (100) **"설치본 미갱신" 은 먼저 `git diff --shortstat <tag>^{commit} HEAD` 로 미릴리스 여부부터 재라** — 캐시 라벨(dir 이름 4.59.0)과 내용(4.61.0)은 다르다. (101) **플러그인은 split worktree 를 만들지 않는다**(`split.md:274`, `SKILL.md:96`) — gotcha 94 는 forkPoint 기록 + 절차로만 닫힌다. (102) 첫 스폰 팀원 3인이 동시에 세션 한도로 죽으면 같은 이름으로 재스폰하면 된다(`-2` 접미가 붙는다). (103) **릴리스 순서는 `ci/**` 푸시 → CI green → master → 태그다.** 태그를 master 착지 전에 먼저 밀면 `release.yml` 의 badge-sync 가 옛 `origin/master` 기준 브랜치를 만들어 fast-forward 가 실패하고 이슈가 열린다(#120 실발생, 2026-09-14 v4.62.0 — 로컬 동등 커밋 `f52b0a98` 로 해소). `git push -u origin HEAD:refs/heads/ci/…` 는 `-u` 가 로컬 `master` 의 upstream 을 그 ci 브랜치로 바꿔버리니 절대 `-u` 를 쓰지 마라. (104) **설치본에 `schemas/` 디렉터리가 4.62.0 까지 한 번도 실린 적이 없었다** — `install.sh#install_hooks`/`install_marketplace_mirror`/`install_plugin_cache` 와 `install.ps1` 의 3개 동형 함수가 `hooks`·`scripts`·`lib`·`output-styles` 만 복사하고 `schemas` 를 빠뜨렸다. `lib/runtime/event-writer.js#getAllowlist()` 가 `schemas/ledger-events.allowlist.json` 을 못 찾아 빈 허용목록 `{}` 로 폴백 → `session.ended` 포함 **모든 ledger 이벤트가 unregistered-event 로 영구 거부**되고 있었다(WIP 이슈가 아니라 릴리스마다 반복되는 구조적 결함). Wave 9 `session-end-vocab` 줄기가 코드를 착지시켜도 설치본이 절대 못 받는 이유가 이것 — "재시작하면 분모가 쌓인다"던 이전 절 전제(line 9)가 **틀렸다**. `df85f702` 로 3개 설치기 함수 + 회귀테스트(`install-files-smoke.test.js`) 수정, `sync:local` 재실행 후 설치본 `getAllowlist()` 39종·`session.ended`/`verify.completed` 등록 실측 확인 완료. **다음에 새 `schemas/*.json` 을 추가할 때 이 3개 함수(및 ps1 동형)에 새 파일명을 나열할 필요는 없다** — 디렉터리 단위 복사라 자동 포함된다.

**다음 할 일**: ① ~~호스트 재시작 후 라이브 확인~~ → **완료(2026-09-14, 세션 continuing artibot-16)**: 재시작 없이도 원장 실측으로 근본원인 확정 — `session.ended` 라이브 0·`ledger.rejected(unregistered-event)` 1건 발견 → gotcha 104 의 패키징 버그가 원인, `df85f702` 로 수정·재검증 완료. **호스트를 아직 재시작하지 않았다면 재시작 후 프롬프트 1개로 `session.ended`/`verify.completed` 라이브 ≥1 재확인은 여전히 유효한 절차 5**(이번엔 통과할 것으로 기대되지만 실측 전까지는 미확인). ② L5 V5-BACKLOG 갱신은 이전 세션에서 완료. ③ `/split plan` Wave 10 — 브리프 승격 3건(창 1·2·3) 먼저, stem 테스트 allowlist 선등록, 시작은 한도 리셋 직후. ④ Observe 판정은 4.63.0(Wave 10) 뒤 라이브 ≥1릴리스 — **이번엔 패키징 버그가 고쳐졌으니 실제로 분모가 쌓일 가능성이 높다**.

### 4.63.0 릴리스 체크리스트 초안 (2026-09-14, 실행은 실제 릴리스 세션에서 — 지금은 초안만)

**설치본 실측 메모 (2026-09-17 릴리스 세션, gotcha 100 보강)**:
(a) **`sync:local` 은 `~/.claude/plugins/cache/artibot/artibot/*/` 의 `.claude-plugin/plugin.json` 을 갱신하지 않는다 — `package.json` 만 덮는다.** 실측: 캐시 7개 dir 의 `package.json` 은 전부 `4.63.0` 인데 `plugin.json` 은 dir 이름과 같은 옛 버전(4.51.0 ~ 4.59.0, `4.50.0` dir 은 파일 자체 부재)이고 mtime 이 2026-08-28 ~ 2026-09-11 로 오늘 무접촉이었다. **캐시 버전 판정은 `package.json` 기준으로 하라** — `plugin.json` 이나 dir 이름으로 판정하면 "설치본이 낡았다" 는 오판이 나온다(gotcha 100 이 말한 "라벨 ≠ 내용" 은 `package.json` 에서만 참이다). 직접 설치본 `~/.claude/artibot/package.json` 은 `4.63.0` 으로 정상 갱신됐다.
(b) **marketplace 미러 `~/.claude/plugins/marketplaces/artibot/` 는 `sync:local` 소관이 아니다** — `claude plugin marketplace update artibot` 이 갱신한다(AGENTS.md §8 명시). 2026-09-17 기준 그 미러의 artibot 항목은 여전히 **4.59.0** 이고 **이번 세션에서 실행하지 않았다**(미실행 — 필요 판단은 오너 몫). 참고로 설치본 `lib/core/model-catalog.js` 의 sonnet id 는 직접 설치본·캐시 7개 dir 전부 `claude-sonnet-5` 이지만 8개 파일 mtime 이 모두 2026-09-15T08:46:59Z 라 **이번 sync 가 넣은 것이 아니라 09-15 sync 가 이미 넣어 둔 값**이다.

- [x] **구 원장 파일 삭제 — 완료 2026-09-15 17:49**(ADR-011 절차 ⑤, 오너 결정 O4): `.artibot/runtime/ledger.jsonl.pre-adr011`(204,406B, 349행) · `.git/artibot/ledger.jsonl.live-premerge-20260914`(442,576B, 389행) · 리더 백업 `.git/artibot/ledger.jsonl.bak-20260915-testrows`(1,005행) 3개 제거. gates 실측 — 라이브 포함 검사 NOT-in-live **0 · 0 · 2**(예외 2행은 의도적으로 제거한 `test-hc`·`test-hc2` 시험 행), 삭제 전후 라이브 원장 불변, 손실 카운터 0.
- [ ] `plugins/artibot/.artibot/runtime/ledger.jsonl`(28행, 7,469B) — **이월(2026-09-15)**. "프로브 잔재" 가설은 **기각**됐다(10일·8세션에 분산). 이관 스크립트 `migrate-ledger-adr011.mjs` 는 **concat 전용**이라(dry-run·append·dedupe 없음) 그대로 쓸 수 없고, 이 28행은 `idempotency_key` 0 · 라이브 원장 미포함이라 **보존**한다. 처분은 다음 릴리스.
- [x] **ADR-011 행 갱신 — 완료 2026-09-15**: `V5-BACKLOG.md` §3 의 ADR-011 행 status 가 `**done(이관) / 삭제 대기**` → **`done`** 이 되고 삭제 실측이 evidence 에 들어갔다(행 위치는 :173 이 아니라 현재 **:177**).
- [x] **패키징 회귀 재확인 — 완료 2026-09-15**: 항목 원문이 세운 조건("`df85f702` 가 `install-files-smoke.test.js` 에 회귀 가드를 넣었으므로 `npm run ci` 통과가 곧 충족 증거")을 그대로 충족했다. 근거 = `c88e699d` 의 **CI Validate 4종 green**(`npm run ci` 에 `validate-install` 포함, 리더 `gh run list` 18:3x 실측) + 로컬 `tests/firewall/install-files-smoke.test.js` **14 passed / 1 skipped**(gates 15:33). 즉 `schemas/ledger-events.allowlist.json` 실재 단언이 CI 에서 돌았다는 뜻이고, **신규 머신·sandbox HOME 설치 스모크를 따로 돌린 것은 아니다**.
- [x] **릴리스 순서 준수 — v4.63.0 완료 (2026-09-17)**: `ci/release-4.63.0` 푸시 → CI green → master ff → 태그 (gotcha 103, `-u` 금지). 순서를 지켰고 **#120 유형 badge-sync ff 실패는 재발하지 않았다**(`gh issue list --state open` 0건). 실행 기록(전부 UTC 실측) — 릴리스 커밋 SHA: **`b7ac9a55`**(선행 docs 커밋 `5719abf9`) · `ci/**` 푸시 시각: **00:37:39Z**(브랜치 `ci/release-4.63.0`, 착지 후 원격 삭제) · CI green run id·시각: **35167319201 CI success 00:46:00Z** + **35167319200 Plugin Validation success 00:38:03Z**(master 재검증 **35168050267 success 00:56:43Z**) · master ff SHA·시각: **`b7ac9a55`**, ff **00:47:56Z** / push **00:48:33Z** · 태그 `v4.63.0` 푸시 시각: 생성 **00:48:41Z** / push **00:49:14Z**, `release.yml` run **35168091056 success**(잡 3/3 — Validate before release · Create GitHub Release · Sync README badges). badge-sync 브랜치 `ci/sync-badges-v4.63.0`(`e92092e5`, 4 files +5/−5)은 **자동으로 master 에 착지**했고 PR 0건 — 그 커밋이 `qualityMetrics.tests` 17,377 → **18,504** 와 문서 산문 3곳을 동기화했다(CI 로그 실측 `Test Files 710 passed / Tests 18,463 passed · 41 skipped (18,504)`).
- [ ] Observe ③④ 라이브 분모 재집계 — **부분(2026-09-16 18:00 KST, 리더 재측정 = investigator 17:48 측정과 일치)**: ③ `verify-rate.mjs --cwd .` → verify.completed 356행 / hook 발화 89 / 18세션, `ids.measured` 0 · `self_report` 0 · `firings.measured_rate` 0(분모 = 전 기간 89 발화). ④ `session-coverage.mjs --cwd .` → session.ended 5 / with_receipts 5 = **1.00**(전 기간, 5행 모두 09-15 09:06~11:10 KST). t0 = 재부팅 2026-09-16 01:31 KST 이후(`--since 2026-09-15T16:31:52Z`) → ended 0 / with_receipts 0 / coverage null, verify-rate lines.total 0 — 재부팅 뒤 라이브 분모 **0**. `outcome-census.mjs` → missions 88 / declared 0 / by_block_code `{}`(원장에 `mission.completed` 0행이라 설계상 null, 핸드오프 기대 STATE_ROW_ABSENT 미출현). 기준 7 "라이브 measured ≥1" 은 편집 턴 부재로 구조적 0(`dev-verify-gate.js` `getChangedFiles()` 0 → return) — 이 줄 편집 뒤 Stop 은 `unmeasured`(`deterministic-source.js#deterministicLayerFrom` stale 분기, 핀 해시 `f51a647bdd7d` 로 확정) — **절차 교정(investigator 18:1x)**: 핸드오프의 "npm test → 편집 → Stop" 순서는 F1(TTL 없음, 결과 timestamp ≥ 마커 mtime) 때문에 100% stale. 올바른 순서 = **Edit 도구 편집 → `npm test`(스크립트 경유만 `runtime/last-test-result.json` 갱신, `npx vitest` 직접 호출은 안 씀) → 추가 편집 없이 Stop**, 그리고 fingerprint(`HEAD|정렬된 changed-set`)가 직전 발화와 달라야 재발화. **기준 7 충족(2026-09-16 18:19:56 KST, 리더 실측)**: 교정 순서(Edit → `npm test` 710 파일/18,492 passed/0 failed → 커밋 a3da15fa 로 HEAD 이동 → Stop)로 deterministic 층 `pass`(evidence 1) → `verify-rate.mjs` `firings.measured` **1** / hook 92, `ids.measured` 1, `measured_rate` 0.0109(18:20:05 측정). **신규 차단(핸드오프 누락, 해소)**: `tests/hooks/route-observe-pre.test.js` `HEAD_ABSENT_RECEIPT.mission_id` 날짜 고정 픽스처가 09-16 부터 결정적 실패(1 failed / 53 passed 단독 재현) → 수리 전 `npm test` red. 미확인: 이 편집 뒤 Stop 이 measured 로 잡힐지.
...
**Wave 8 착지 (split-68e984w8, 8창 1 wave, 8/8줄기)**: 착지 순서 base `a40b8448` → p1 **`e9e24e2e`**(checkpoint-store-dr01 · plugin-manifest-agents) → p2 **`34b6dbf6`**(pricing-unify) → p3 **`b906264a`**(context-receipt-cx02) → p4 **`9300568e`**(intent-md-generator · routebench-baseline · verdict-verify-writer + README 카운트 동기화 `c23eb5c4`) → p5 **`ee86a639`**(human-resolved-writer) → docs **`34940d2b`**(V5-BACKLOG 39% + CHANGELOG [Unreleased] Wave 8 절). `git diff --shortstat a40b8448 ee86a639` = **74 files +14,989/−277**(리더 실측, 2026-09-13 23:4x). 창 운영: 21:4x 에 창 8개 전부 사망 상태에서 5창 재개설(`claude --worktree` 가 기존 worktree 를 재사용함을 실증) → dispatch 재발행 → 5줄기 착지까지 약 65분(21:57 포인터 → 22:57 p5).

| 줄기 | done SHA | 파일 | 핵심 (1줄) |
|---|---|---|---|
| checkpoint-store-dr01 | `40a8d21d` | 11 files +1,996 | `lib/checkpoint/{checkpoint-store,checkpoint-service,checkpoint-validator}.js` + `adapters/` 신설, `contracts.js#validateCheckpoint`, 불변성 게이트 — SH-21 DR01 done, **DR02 미착수** |
| plugin-manifest-agents | `007b527b` | 7 files +391/−20 | `plugin.json#agents` 28→30(auditor·investigator) + `tests/ci/agents-manifest-parity.test.js` 패리티 게이트, "28 agents" 산문 5파일 정정 — §3 부채 1건 done |
| pricing-unify | `d6cccd0c` | 8 files +1,156/−82 | `lib/core/model-catalog.js` 단일 가격표를 usage-receipt·cache-roi 가 읽고 `pricing-parity.test.js` 가 4소비처를 한 표로 고정 — SH-07 done(5.1 계수 실측 잔여) |
| context-receipt-cx02 | `fc21e99e` | 10 files +2,029/−81 | `context-pressure.js`(호스트 `context_window.max_tokens` 없으면 null, 카탈로그 폴백 제거) · `context-receipt.js` · `rehydration.js#reportContextReceipt` + PostCompact 배선 — **SH-16 in-progress 유지**(필수 10키 중 6만 채움 → 라이브 발행 0 은 구조적) |
| intent-md-generator | `7d75031a`(1차 done `17f3e87c`) | 10 files +2,252/−42 + 핀 갱신 2 | PreToolUse `Write\|Edit` 훅 `intent-observe-pre.js`(stage-② S1 → `mission.created` + `missions/<id>/intent.md`, stdout 바이트 불변 3케이스) + `artifact-lifecycle.js#apply` 게이트 3 writer — SH-01 done, 베이스라인 created/(created+deferred)=**20/243=8.23%**(배포 전) |
| routebench-baseline | `560d1bce` | 6 files +2,221 | `scripts/bench/routebench.mjs` 오프라인 채점 러너 + `baselines.{json,schema.json}` B0~B6 + `scenarios.schema.json` 선택 `agentType` — SH-25 done |
| verdict-verify-writer | `7298bc34` | 11 files +3,443/−10 | `lib/review/{verdict-writer,claim-audit-writer}.js` · `lib/verification/verify-writer.js` + SubagentStop 배선 `_review-stop-record.js` — OB-06 done, **OB-07 in-progress(런타임 호출자 0)** |
| human-resolved-writer | `c20ca01f` | 5 files +1,475/−29 | `human-asked-record.js#recordHumanResolved`(kind 3, 스킵 5, 3,072B 사전검사) + CLI `scripts/ledger/record-human-resolved.mjs` + project.md 규약 7줄 — SH-27 done, 호출 주체 아직 0 |

**p4 1차 not-green 원인**: 1차 `ca0f0da7` 에서 `tests/hooks-schema-shape.test.js` 6b `PRETOOLUSE_INTENT` 누락 + 지문 불일치(intent 가 추가한 hooks.json 슬롯) 2건. 줄기가 `7d75031a`(`WRITE_TOOLS` 4종, 지문 `4e0039e6…`) 로 수리 → 2차 초록. **리더 plan 결함**: 그 핀 테스트가 plan allowlist 에도 표적 스위트에도 없었다 — land 의 `git merge-tree` 와 표적 스위트는 **파일 간 핀**을 못 본다.

**V5-BACKLOG 진행률**: §1 합계 **33% → 39%**(done 35 → 39 = SH-01·SH-25·SH-27·OB-06 추가, Shadow 3 → 6). done 으로 올리지 않은 것 3건 — SH-16(writer 착지, 라이브 발행 0 구조적) · SH-02(intent.md 만, review/outcome.md 핸들러 실행 0) · OB-07(verify.completed writer 착지, 런타임 호출자 0).

**정리 완료(2026-09-13 23:3x~23:4x, HANDOFF 원문)**: worktree 8개 전부 제거(`.claude/worktrees`·`.git/worktrees` 비어 있음, 부모 `node_modules` 107 무사) · 로컬 브랜치 `worktree-split-artibot-*` 20 + `docs/wave8-*` 2 + `tmp/wave8-p4-preview` 삭제 · 원격 `ci/split-68e984w8`·`ci/split-wave8-final-docs`·`ci/split-wave8-p2-docs` 삭제(세 tip 모두 `origin/master` 조상 확인 후).

**리더 오류 7건과 정정(전부 창이 정정, 재발 방지는 메모리 2건)**: ① Git Bash `tasklist /FI` 경로 변환으로 pid 를 전부 사망 오판 ② 메시지 시각 추정이 20~30분 과대(22:12 KST 이전 타임스탬프 전부) ③ human-resolved 브리프가 부재 파일 `commands/ask.md` 인용(정답 `.artibot/project.md#Human Approval Boundaries`) ④ `record-review.mjs +1` · README `72→73` 전제가 틀림(실제 카운트 변화 없음 / 70→72) ⑤ cx02 "model-catalog 에서 ctxLimit 를 읽어라" 는 PostCompact 경로에서 불가능(페이로드에 모델 티어가 없다) ⑥ 6b 표 지시를 `['Write','Edit']` 로 냄(정답 `WRITE_TOOLS` 4종) ⑦ routebench 브리프 "신규 테스트 1"(실제 2 — 픽스처 + 러너).

**gotcha 신규**: (89) Git Bash 에서 `tasklist /FI "PID eq N"` 은 `/FI` 가 `C:/Program Files/Git/FI` 로 경로 변환돼 **항상 실패** — 모든 pid 가 죽은 것으로 읽힌다. PowerShell `Get-Process` 또는 `MSYS_NO_PATHCONV=1` 을 써라(첫 세션의 "창 8개 전부 사망" 결론은 tasklist 가 아니라 ListAgents 0 이 뒷받침한 것이었다). (90) **README 카운트 동기화는 리더가 병합 미리보기에서 1회** — 3줄기가 동시에 훅·스크립트를 늘리면 줄기마다 고칠 때 충돌한다(Wave 8 은 `c23eb5c4` 로 훅 등록 27→28 · 훅 스크립트 70→72 를 리더가 한 번에). (91) **설정 파일(`hooks.json`·`plugin.json`·`dispatch-table.json`)을 건드리는 줄기는 그 파일의 핀 테스트를 plan allowlist + 표적 스위트에 넣어라** — land 의 `git merge-tree` 는 파일 간 핀을 못 보고, 배치 랜딩 1차에서 not-green 으로만 드러난다(gotcha 14/23 계열). 후속안: `land.mjs` 가 변경된 설정 파일의 리터럴 경로로 테스트를 grep 해 "실행할 핀 테스트" 를 나열. (92) `resolveDispatch` 가 **이미 착지·정리된 줄기에 refused 를 낸다** — done 줄기 제외가 판정 **뒤**에 오기 때문(`lib/git/split-dispatch.js#resolveDispatch`). 정리 후 dispatch 를 다시 부르면 거짓 경보다. (93) `git worktree remove` 가 Permission denied 로 실패하면 PowerShell 에서 실행하라(Git Bash 경로/락 처리 차이).

**소유 밖 후속(줄기별 1줄 — `.artibot/split/followups-wave8.md` 는 gitignored 라 여기가 유일한 추적 사본)**:
- **intent-md-generator**: allowlist `mission.candidate_deferred.fields` 에 `title` 추가 · `event-writer.js#sessionFallbackMissionId` 8자 접두 충돌(프로브 재현) · hook-decision-invariance import 실패 일반화 · S2 Bash commit/push 는 Wave 9 · `created` 와 `state.updated` ts 6ms 역전(buildEnvelope ts) 원인 미규명 · 테스트 파일 833줄(800줄 경고선 초과, 나중에 분할) · `enabled:false` 여도 Write 마다 원장을 한 번 통째로 읽는다.
- **verdict-verify-writer**: `verify.completed` 런타임 호출자 0 → dev-verify-gate 배선은 별 줄기(CA-13) · `existingKeys` 가 리뷰어 stop 마다 원장 전체를 읽는다(239행, 지연 미측정 — 커지면 tail-read) · 마지막 어시스턴트 턴이 tool_use 뿐이면 `skipped:no-text`(설계 결정 필요, 지금은 의도된 가시적 구멍) · `team.md` v2+claim_audit 준수율 라이브 0(배포 후 지표).
- **context-receipt-cx02**: `scripts/hooks/context-tracker.js:21` 이 `128_000` 하드코딩(카탈로그 ctxLimit 와 불일치) · 라이브 PreCompact 페이로드의 `context_window` 존재율 미측정(없으면 압력 점수 항상 null).
- **routebench-baseline**: 봉투에 `policy_source.config_path` 미기록(`CLAUDE_PLUGIN_ROOT` 가 러너를 설치본 config 로 돌릴 수 있다) · `baselines_sha256` 이 바이트 수준이라 CRLF 로 머신마다 달라짐(로컬 결과 전용) · B3 정적표가 `security-reviewer` 를 fable 로 분류(denylist 는 `resolveModel` 전용 — 결함이 아니라 드러내야 할 괴리).
- **human-resolved-writer**: `kind_source` 키가 allowlist 에 없음(enum 강제 없음) · `data.path` 길이 미검사(>721B 는 여전히 `ledger.rejected`, 문서화됨) · CLI 호출자 0 · `ledger-fold` 가 상한 미만에서도 접는 정황(라이브 `human.asked` 최대 3,363B < 4,096 cap → `foldOversized` 가 캡 아래에서 접을 수 있음) · 호스트가 AskUserQuestion 에 PostToolUse 를 발화하는지 미확인.
- **checkpoint-store-dr01**: DR02(재개 주입) 미착수 — SH-21 잔여.
- **pricing-unify**: 5.1 비용 계수 2.6× 실측 미수행.
- **리더·인프라**: `resolveDispatch` 완료줄기 선제외(gotcha 92) · `land.mjs` 설정 파일 핀 테스트 나열(gotcha 91) · 설치본 4.59.0 은 spawn 원장을 레거시 `.artibot/ledger/` 에 쓰고 master 는 `.git/artibot/`(W7 `4325390e`) → 레거시 경로 판독기(doctor.md 등)는 `claude plugin update` 전까지 0 을 본다.

**오너 결정 3건(릴리스 전 처리) — 결정 완료, 릴리스 반영 완료(`7dd14049`)**. 반영 형태는 항목마다 다르다: ① 은 config·핀 테스트로 실제 출하됐고, ② 는 "변경 0" 결정이라 코드 변경이 없으며, ③ 은 Wave 9 줄기 `routebench-b4-agenttype` 로 넘어갔다.
1. **`runtime.artifactLifecycle.enabled` 기본값** — (a) ON 유지(설계 SH-01 "생성 시작", Shadow) / (b) OFF 로 출하 + 문서. 쟁점: 설치본 갱신 뒤 **모든 프로젝트**가 첫 `Write|Edit` 에 `.artibot/missions/<id>/intent.md` 를 만들고, `.artibot/missions/` 는 설계상 **추적 경로**(`.gitignore:119` 주석이 tracked 로 명시) — 사용자가 새 추적 파일을 보게 된다. 줄기를 막는 사안은 아니고 릴리스 노트에서 결정.
   → **권장안(결정 2026-09-14 ①): 4.61.0 은 `false` 로 출하.** 근거 — 설계 §7.3(:403) "Shadow 에서 시작, Observe substantive 분포 선행", V5-BACKLOG §4 Observe 종료 판정 **0/5**, config 주석(`artibot.config.json:1100`) 이 false 를 "Observe-stage behaviour" 로 명시. 프로젝트 단위 오버라이드가 없어(`lib/core/config.js:120` 단일 경로) Artibot 리포 자신도 OFF 가 된다. ON 전환은 **Shadow 진입 릴리스의 config 1줄**. 변경 범위 = config 1줄 + 핀 테스트 2(`artifact-lifecycle.js` apply :521-523, dryrun :797). 후속(Wave 9 후보, 설계안 선행): 프로젝트 단위 게이트(`intent-observe-pre.js:545` 가 config AND `.artibot/project.md` 존재를 보거나 `loadConfig` 로컬 오버라이드)와 설계 :284 결정 B4(훅이 승격한 mission 을 "사람 mission" 으로 볼 것인가)를 **함께** 닫는다.
2. **`schemas/ledger-events.allowlist.json` 의 `context.compiled.sources` 에 hook 추가 여부** — 현재 `["worker"]`, 설계 §8.2 는 첫 writer 를 PostCompact 훅 경로로 본다. 핀: `tests/hooks/runtime-prompt-memory-instrumentation.test.js:190-200` 이 hook 소스를 거부(2026-09-12 실측). Wave 8 리더 판정은 (b) 거부 유지 — PostCompact 호출자가 영수증 11 leaves 를 못 채워(`post-compact-rehydrate.js` ~:289) 지금 넓혀도 라이브 발행이 0 이다. 영수증을 훅에서 완성할 수 있게 되는 시점에 함께 결정.
   → **권장안(결정 2026-09-14 ②): `["worker"]` 유지, 변경 0.** 근거 — allowlist **앞에 관문이 둘 더** 있다: writer 포트가 null 이고(`post-compact-rehydrate.js`, 고의) → `no-writer-port` → `receipt-incomplete`. 3중 구조라 allowlist 만 넓혀도 발행은 0 이다. 결손 11 leaves = mission_id 1 · based_on 2 · transforms 5 · cache 3. 재개 조건 = **훅 호출자가 10키를 실측으로 공급할 수 있을 때**. 그 시점에 할 일은 allowlist 확장이 아니라 설계 §8.2(:467) "첫 writer = PostCompact" 문구의 정정 검토다(실제 첫 writer 는 worker 측 context-compiler 어댑터).
3. **RouteBench B4 정의 — 베이스라인이 classifier 에 `input:{agentType}` 을 공급할 것인가** — 오늘 class 는 항상 default 라 B4 는 "정책 상한 안에서의 scorer 선택"이고, 라이브 2티어 config 에서도 planner·architect 에 opus 를 권한다.
   → **권장안(결정 2026-09-14 ③): 공급한다 — `routeModel({agentType, config, input:{agentType}})`, Wave 9 줄기 `routebench-b4-agenttype`.** 근거 — 프로덕션 유일 호출자 `scripts/hooks/route-observe-pre.js:255-262` 는 `input:{text, agentType}` 을 넘기는데 현 B4 는 input 이 없어 **항상 `class:default`** 로 떨어지고, 그 결과 allowlist 8종 **전부**에 가짜 divergence 가 생긴다(2026-09-14 00:12 실측: planner·architect·code-reviewer 가 as-is 는 opus, input 공급 시 fable). 변경 파일 4 — `baselines.json` B4 call/note · `routebench.mjs:159-161` · `routebench-runner.test.js:137`·`:306-307`. `baselines_sha256` 은 변동하므로 그 라벨을 붙인다.

**문서 간 수치 불일치(숨기지 않고 기록)**: ① 커밋 `c05200d0` 제목은 "V5-BACKLOG 35%→39%" 인데 CHANGELOG·V5-BACKLOG §1 은 **퍼센트 33%→39%, done 개수 35→39** — 제목이 두 축을 섞었다. ② Wave 8 배정 시 plan base 는 `0c08d8b3`(앞 절) 인데 착지 diff 의 base 는 `a40b8448` — 배정 후 master 가 전진했다(**조상 관계 확인(리더 실측)**: `git merge-base --is-ancestor 0c08d8b3 a40b8448` 참). ③ Observe 재판정의 `review.completed` 분모 **0/487행** 은 리더가 검증하지 않았고, 창 실측은 `.git/artibot` 233행 + 레거시 349행 = 582행(양쪽 모두 `review.*`·`verify.completed` 0) — 487 의 출처 미확인. ④ HANDOFF 표의 intent-md-generator doneCommit 은 `7d75031a`(수리 후), followups 와 CHANGELOG 의 "done `17f3e87c`" 는 1차 done — 같은 줄기의 서로 다른 시점이다.

**인스펙션 후속 F-1 정정 완료**: `README.md` 4.61.0 요약 줄의 "verdict/claim-audit/verify writers wired to SubagentStop" 를 "verdict writer wired to SubagentStop, claim-audit and verify writers landed with zero runtime callers" 로 교체했다 — `scripts/hooks/_review-stop-record.js:16` 은 `verdict-writer.js#recordReviewOutcome` **만** import 하고, `claim-audit-writer`·`verify-writer` 실호출은 `lib/`·`scripts/` 전역 0건(inspector 실측 00:4x, 리포터 재확인).

**다음 할 일**: ① ~~v4.61.0 릴리스~~ → **완료(2026-09-14)**: docs `12adef6e` → release `7dd14049`(태그, GitHub Release 공개) → badges `9948348c`. CI·`release.yml` 전부 success. 체크리스트 이행분 = `runtime.artifactLifecycle.enabled` 를 `false` 로 출하(결정 ①, config 1줄 + 핀 테스트 2) · README 카운트 28/72 · 산문 동기화(`sync:readme:claims`). **`.pre-adr011` 은 여전히 미결**: 파일 0건은 "삭제 완료" 가 아니라 **이관을 안 돌려서 생긴 적이 없는 것**이다(V5-BACKLOG:165 in-progress, migrate 스크립트 실행 흔적 0) — 이관 실행 뒤 삭제(이관 미실행이면 no-op). ② **오너 `claude plugin update` 대기** — 플러그인 캐시 라벨은 아직 4.59.0(cache 내부 `package.json` 은 4.60.0, 라벨 갱신 주체 미확인). 어느 복사본에도 `intent-observe-pre` 가 없으므로 **갱신 시점이 그 훅의 첫 라이브 발화**다(단 `enabled:false` 출하라 파일 생성은 0, 원장·스토어 기록만). ③ 갱신 후 **Observe ③축 재집계**(`review.completed` 분모 — 호출부가 배포되기 전까지는 구조적으로 0) + 실 원장 이관(`scripts/ledger/migrate-ledger-adr011.mjs`, 선존재 exit 3) ④ **Wave 9 배정 확정** — runId `split-68e984w9`, base `7dd14049`, **8줄기**: `resume-contract-dr02` · `verify-gate-wiring` · `review-md-writer` · `session-end-vocab` · `avoided-switch-fold` · `routebench-b4-agenttype` · `guard-command-position` · `split-ops-dispatch-land`. 로드맵 6 : 부채 2(25% 상한 준수), serial 0 · conflict 0, dispatch dry-run **8/8 ok**. **대기 중인 것 = 오너가 창 8개 열기**(`claude --worktree split-artibot-<limb>`) → 리더 worktree-setup 8 → dispatch 8. 브리프마다 "리더 통합 단계" 절(gotcha 90·91)을 필수로 넣는다.

## Wave 7 착지 + Observe 재판정 + Wave 8 배정 (2026-09-11 23:5x KST, 세션 artibot-7b/25918244, master **0c08d8b3**, 설치본 4.59.0 로드 / 4.60.0 sync:local 사본)

**Wave 7 (split-68e984w7, base f75e849f, 4창)**: 4줄기 전부 착지, master **0c08d8b3**(ci/split-68e984w7 단일 SHA, CI 7/7) — frontmatter-hardening 4f432d24 · guard-redos-residual · test-load-flakes · spawn-ledger-store. land 7/7 ×4(lint UNSUPPORTED 1건은 worktree CRLF stat 아티팩트 → 리더가 `git show <sha>:<file> | eslint --stdin` 4파일 0 확인). 원격 ci 브랜치 삭제, run.json integrated. **로컬 정리 대기**: worktree 2개(guard-redos-residual pid 68460 · test-load-flakes pid 75956 — 창이 아직 열려 있음, 브랜치는 master 에 merged) + 로컬 브랜치 12개 `-d`(오너 `!` 실행: docs-comment-parity · doctor-project-name · frontmatter-hardening · guard-normalize · guard-redos-residual · guard-residual · release-claims-sync · schema-frontmatter · spawn-ledger-store · test-hygiene-bench · test-load-flakes · worktree-ledger-store, 전부 `worktree-split-artibot-` 접두).

**오너 피드백 → 워크플로우 원칙 확정(22:3x~23:0x)**: "wave 끝번호가 없다 = PRD 기준 todo 가 없다는 것" · "v5.0 방향으로 1개도 구축 안 되고 결함만 고친 건지 판단하라" · "목표 설계에 따른 작업도 진행돼야 한다, 단 선행 결함이면 이해". 리더 자기 판정(실측): 오늘 12줄기 중 V5-BACKLOG 항목 = 3(전부 Observe 결함 수리), **Shadow 진입 0**. 원인 = 정찰→후속→웨이브 루프 자기 증식 + "실결함" 이면 전부 편입 + 설계 §4 미열람. **새 원칙**(메모리 `feedback-roadmap-driven-waves`·`feedback-split-throughput-first`): ① 웨이브 입력 = `.artibot/guides/v5-design/V5-BACKLOG.md`(로드맵 분모, 100항목) ② 부채·후속은 웨이브당 25% 상한(선행 결함만 예외) ③ 유휴 창 정찰 = 다음 로드맵 항목 브리프 ④ 검수 라운드 ≤2 ⑤ 착지마다 진행률 % 보고.

**V5-BACKLOG.md(신규, 미커밋 — 이 커밋에 포함)**: 기준 SHA f75e849f. §1 진행률 P0 81% · Observe 63% · Shadow 3% · Canary 5% · GA 14% · **합계 33%**. §2 100항목(P0-01~16 · OB-01~27 · SH-01~30 · CA-01~20 · GA-01~07) status/evidence/SHA. §3 부채 트랙. §4 Observe 종료 판정 5축. §5 Shadow 진입 최소 집합 10줄기. §6 다음 웨이브. §7 미확인 21건. 갱신 규칙: 웨이브 착지마다 status·evidence·SHA. **갱신 대기**: 미확인 6건 grep 결과(P0-13 스키마 0 · OB-12 1파일 · OB-17 17건 · OB-21 1파일 · CA-17 2파일 · CA-19 1파일 · SH-10 1파일) + §4 를 아래 재판정 수치로.

**Observe 재판정(investigator observe-judge-opus, 23:2x)**: 인프라 5항 4 PASS + Check 8 FAIL(원인 = ADR-011 이관 잔재 — 구 원장 `.artibot/runtime/ledger.jsonl` 349행 미이관, 병합 드라이런 pass, 유실 0). 설계 §4 5축 0/5 → **2/5**: ① substantive 2/40 = 5% ② 추천≠정책 7/31 = 22.6% ③ 분모 0(`review.completed` 0/487행) ④ `session.end` 어휘 부재로 미측정 ⑤ 구조적 FAIL. UPS 가드 축 5 는 결과 PASS 지만 기전 미확인(호스트 래퍼가 마커에 없음 → fail-open 후보). **정정**: 호스트 로드 설치본은 **4.59.0**(4.60.0 은 sync:local 사본, `claude plugin update` 필요).

**Wave 8 배정(split-68e984w8, base 0c08d8b3, 8창, 23:5x KST)** — 로드맵 7 + 선행 결함 1(manifest, 25% 상한 안): `checkpoint-store-dr01`(SH, `lib/checkpoint/` 신설 + `validateCheckpoint`) · `context-receipt-cx02`(SH, `context.compiled` 영수증) · `intent-md-generator`(SH, artifact-lifecycle apply + PreToolUse observe 훅, 매처 `Write|Edit` 만) · `verdict-verify-writer`(OB-06/07, `review.completed`·`claim_audit`·`verify.completed` writer — SubagentStop 페이로드 실측이 첫 측정, `subagent-handler.js` 단독 소유) · `human-resolved-writer`(SH-27, `human.resolved{kind}` — AskUserQuestion PostToolUse 발화 실측 1순위, 없으면 CLI+규약) · `routebench-baseline`(SH, B0~B4 + `scenarios.agentType`) · `pricing-unify`(OB, 카탈로그 단일 진실원, `cost.total` null 핀 유지) · `plugin-manifest-agents`(부채, `plugin.json#agents` 28→30 + 패리티 게이트, cowork 는 허용목록 루트). plan: profile fast · 8줄기 1 wave · serial 0 · conflict 0 · limits maxWorktrees 8(override). dispatch dry-run 8/8 ok. 브리프 `.artibot/split/<limb>/brief.md` 8건(각 9절 + 리더 결정 + 검수 라운드 상한 2 경고) + 리더 부칙 3건(소유권: `hooks.json` → intent, `commands/team.md` → verdict, human 은 `commands/ask.md`). **Wave 9 후보**: `guard-command-position`(브리프 + leader-addendum 완비).

**리더 전제 오류(창들이 정정, 기록)**: validateCheckpoint 코드 0건(신설이 맞음) · team.md Phase 3.5 는 진행률 렌더링(검수는 4.5) · "세 소비처" 실제 7 · validate-install PARITY_MATRIX 에 에이전트 핀 없음 · INDEX.md·marketplace.json 이미 30 · 6훅은 이미 toplevel 전달. 브리프마다 "내 인용이 틀렸으면 틀렸다고 보고하라" 명시.

**gotcha 신규**: (86) 크로스세션/서브에이전트 메시지는 ~4~5KB 에서 절단 — 브리프는 절 단위로 분할 재전송 요청. (87) Fable 한도로 architect·investigator 가 착수 직후 죽으면 opus 로 재스폰(이번 4명). planner 에이전트는 Write 도구가 없다 — 본문을 메시지로 받아 리더가 Write. (88) plan.json 을 만들 때 브리프 "소유 파일 allowlist" 절을 그대로 옮기되, 두 줄기가 같은 파일을 들면 `buildFastFanoutPlan` 이 conflictGroup 으로 묶어 serial 로 뺀다 — 한쪽에만 배정하고 다른 쪽 브리프에 부칙.

**다음 할 일**: ① 오너: Wave 7 창 2개 닫기 → 리더가 worktree 2개 정리(unlock/remove/prune) + 오너 `git branch -d` 12개 ② 오너: Wave 8 창 8개 열기(`claude --worktree split-artibot-<limb>`) → 리더 worktree-setup 8 → dispatch 8 → run.json opened/dispatched ③ NEXT-SESSION + V5-BACKLOG + CHANGELOG [Unreleased] Wave 7 항목 커밋 → ci 경유 랜딩 ④ Wave 8 착지마다 V5-BACKLOG status·% 갱신 ⑤ v4.61.0 = Wave 7+8 착지 후(`.pre-adr011` 삭제 체크리스트 포함) ⑥ 실 원장 이관(창 전부 닫힘 + `claude plugin update` 4.60.0 뒤) ⑦ 오너 판단 대기: `runtime/user-profile.json` 벤치 오염 삭제 · `~/.claude.json` sha 변경 주체 · UPS 가드 fail-open 후보 · Observe ③⑤ Shadow 이월 · `session.end` 어휘 추가 · `human.resolved` allowlist source 추가(cx02 브리프).
...
**v4.60.0 릴리스(18:0x KST 완료)**: docs a5dcaf96 + release 8eb41581 을 ci/wave6-docs-release 한 번으로 랜딩(CI 7/7) → 태그 v4.60.0 → release.yml run 34580595217 validate·release·sync-readmes 전부 success. **W5-d 라이브 실증 성공**: 배지 커밋 f75e849f 가 marketplace.json + 산문 3파일(CLAUDE.md·AGENTS.md·MARKETPLACE-SUBMISSION.md) 을 함께 실었고(4.58.0·4.59.0 은 1파일), 4 validate 잡 드리프트 0. 그러나 Windows 잡 1건 실패 → #119: `tests/autopilot/safety.test.js` 선형성 벽시계 단언 `50.5407 < 50`(러너 경계값 플레이크, Linux 3잡·master 같은 잡은 통과) → 그 잡만 재실행 초록 → 리더 ff f75e849f → #119 닫음. 설치본 4.60.0(`sync:local`), 리눅스 러너 계수 15,968 = 리더 로컬 계수 일치. **gotcha (85)**: 벽시계 `<50ms` 단언은 CI 러너에서 경계값 플레이크 — 구조 단언(무제한 런 부재·창 바운드) + 여유 4배로 바꿔라(Wave 7 guard-redos-residual 에서 적용).

**Wave 7 정찰 추가 발견(guard-residual 창, 18:5x)**: L1 `blocked-patterns.js:36-37`(rm 2규칙 `.*\/`)·`:280-281`(wget/curl pipe `\s+.*\|`) + L2 `safety.js:197`(sql-delete `(?!.*\bWHERE\b)` /s) 가 **무제한 런 2차식** — curl pipe 40,962B 1.4~1.5s, 122,880B 13.3s(**PreToolUse 5초 예산 2.7배 초과**, 비율 20.2×/9.1× 로 2차식 확정). 4.60.0 의 `{0,192}` 컨벤션이 L2 4계열에만 적용됐고 이 5규칙은 잔여. → Wave 7 줄기 `guard-redos-residual` 로 분리(설계 의존 없음), `guard-command-position` 은 같은 파일이라 Wave 8.

**Wave 7 배정(split-68e984w7, base f75e849f, 4창, 19:4x KST)**: spawn-ledger-store · frontmatter-hardening · test-load-flakes · guard-redos-residual. Wave 6 worktree 5개 정리 완료(pid 사망 확인 → unlock/remove/prune, junction 5 해제, node_modules 107 유지). 22:1x 기준 진행: frontmatter-hardening **done 4f432d24 land 7/7**(빈 description 9셀 RED 게이트 + 파서 fold; 창이 자기 변경의 source_hash 드리프트를 잡아 재커밋) · guard-redos-residual A/B 착지(rm 2규칙 `{0,512}` — 리더 결정: 513자 초과 `rm --recursive` full-stack 사각을 문서화로 넘기지 않음, pipe 2규칙 `{0,192}`, 비율 임계 → 한 구간 `t(122,880) < 18×t(20,480)`, 정적 스캔 65규칙 위반 0) 검수 대기 · test-load-flakes(리더 조건부 승인 → 창이 훅 예산 표로 **타임아웃 상향 철회**, git-dir.js 레이아웃 추적 1순위로 스폰 0 — 6훅은 이미 toplevel 을 넘김) 재작업 중 · spawn-ledger-store wip 2(B3 docs·B1 코어), 드라이런 C(선존재 exit 3 경로) Σ 434 일치.

**Wave 8 후보(브리프, 부모 `.artibot/split/`)**: `guard-command-position/brief.md` + `leader-addendum.md`(프린터 세그먼트 면제 전처리 (c) 채택 — 적대 회귀 0/67·PARITY 무반전·정규식 0 편집; 추가 목표 L2 recursive-only rm 규칙·human-gates.js:179,181 무앵커 측정; W7 guard-redos-residual 착지본 위) · `frontmatter-followups/brief-draft.md`(89줄 6목표 — **실결함**: `plugins/artibot/.claude-plugin/plugin.json#agents` 28항목 vs `agents/*.md` 30, `auditor`·`investigator` 누락. 이 머신은 `~/.claude/agents/` 사본(리더 ls 2/2)으로 스폰이 됐고 플러그인 네임스페이스 `artibot:` 은 28 만 노출 → 신규 설치에서 `Agent(subagent_type:"investigator")` 실패 가능(추론). 매니페스트↔디렉터리 동일 게이트 + 누락 등록 + 호스트 실측 설계. 그 외: skill-authoring §3 모순 4문장, `validate-agent-frontmatter.js` 소비처 0(삭제 vs CI 편입 오너 결정), `source-hash-fresh.test.js` 114/114 핀, 빈 description 호스트 실험 설계).

**설치본 관측(22:0x)**: 4.60.0 설치본이 이미 `.git/artibot/ledger.jsonl`(67KB) 을 쓴다 → 수동 이관 게이트는 ADR §절차 "선존재(exit 3)" 경로 확정(옛 `.artibot/runtime/ledger.jsonl` 346행 + worktree 원장 + 새 공유 파일을 마지막 `--worktree` 입력으로). spawn-ledger-store B4 드라이런이 이 케이스를 재현(Σ 434, dedupe 0).

**다음 할 일**: ① Wave 5·6 worktree 8개 정리(창 닫힌 뒤: teardown → pid → unlock → remove → prune) + 로컬 브랜치 삭제(오너 `-d`) ② `/split plan` Wave 7 = spawn-ledger-store · frontmatter-hardening · test-load-flakes · guard-redos-residual(4줄기, 소유 겹침 0 확인 필요) ③ Wave 8 = guard-command-position(W7 guard-redos-residual 착지본 위) ④ 실 원장 이관(창 전부 닫힘 + 설치본 갱신 뒤 리더 수동 1회: `scripts/ledger/migrate-ledger-adr011.mjs` 로 `ledger.jsonl`·`spawns.ndjson` 각 1회, 선존재 exit 3 함정 = 설치본 갱신 후 첫 스폰이 새 파일을 만들면 그 파일을 마지막 `--worktree` 입력으로) ⑤ `.pre-adr011` 삭제는 v4.61.0 체크리스트.
