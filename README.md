...
[![Version](https://img.shields.io/badge/version-4.66.0-blue?style=flat-square)](plugins/artibot/CHANGELOG.md)
...
**4.66.0** (2026-09-22) — Ships `/split` Wave 16 (8 limbs) and Wave 17 (10 limbs closed = 9 landed + one code-zero verdict) together: 62 commits, 66 files +9,097/−218 since v4.65.0. **Two deliberate behavior changes**, both narrowing rather than widening. (1) `scripts/ledger/record-verify.mjs` and the human-resolved recorder now fall back to `CLAUDE_CODE_SESSION_ID` when `CLAUDE_SESSION_ID` is empty — the host supplies the former, so the old wording recorded nothing even when followed exactly (`recorded:false, reason "no session_id"`); `commands/verify.md` promotes the call from a Pipeline Behavior bullet to a numbered **Step 5** that takes `--cwd`. Live `self_report > 0` still needs this release plus a plugin update plus a host restart. (2) Guard grades more `rm` targets as dangerous across two limbs: `guard-l2-residual` promotes brace-form `${HOME}` (5 shapes) and `~name` / `~+` to `danger` (39 grades up, 0 down over 651 strings, no new rules), and `guard-rm-flag-redos` covers the remaining tilde set. Net against the pre-limb baseline over a 25,483-string corpus: **716 up, 0 down**. Against the immediately preceding state, four shapes (`~+`, `~-`, `~1`, `~1abc`) move `danger` → `caution` on purpose, aligning them with `.` and `$PWD`. Everything else stays OFF or opt-in: the three `V5-BACKLOG` §4-b keys remain `false` and `routing.canary.actionClasses` stays empty; the artifact-lifecycle gate **tightens** to `resolveArtifactGate` (global `enabled` AND a per-project `.artibot/artifact-lifecycle.optin` marker, fail-closed, resolved in exactly one place), so flipping the global switch no longer writes into every project; `/resume --read-order`, the autopilot fast objective metadata (instruction-level `limits.applyObjective`, not a config key) and the `/scorecard` hold/residency fold are opt-in with default transitions deferred by owner decision. New observation surfaces, all read-only or test-only: `scripts/ledger/verify-call-rate.mjs` (two carrier denominators, live `null` because numerator and denominator are both 0), `session.ended` `no-receipts:<no-entries|all-synthetic|all-unresolved|no-usage>` cause suffixes, a writer-idempotency inventory firewall (brief inventory 10 corrected to 6 keyed modules against 53 scanner-visible appenders), and a real-`StateStore` read-path pin for split state. Also: `lib/core/file.js#atomicCreateTextSync`, an exclusive-create primitive (190 double-writes in 200 contended rounds today vs 0 with the primitive) — the `artifact-lifecycle` caller has **not** adopted it yet, so that TOCTOU is still open. Security fix: the quadratic `rm` flag lookahead is gone. A language-preserving token swap (`-[a-z]*[r][a-z]*` → `-[a-qs-z]*r[a-z]*`) across 8 rules (4 in L2, 4 in L1) takes `classifyRisk` from 1,986.56ms to 0.83ms on a 20,000-`r` run, and from 15,865ms to 3.0ms on a 40,962-byte one, with 0 mismatches across 2,396,736 enumerated cases. This matters because a timed-out PreToolUse hook does not block the tool call. Known and unfixed: the `git-branch-delete` rules carry the same quadratic shape (120.4ms at a 10,000-character run) — the `-D` case asymmetry needs a different swap, so it is a follow-up limb. Roadmap progress §1: 47% → 48%.
