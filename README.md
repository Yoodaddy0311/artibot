...
[![Version](https://img.shields.io/badge/version-4.28.1-blue?style=flat-square)](plugins/artibot/CHANGELOG.md)
...
|---|---|---|---|
| [`artibot`](./plugins/artibot/) | Claude Code (developer CLI) | **4.28.1** | full Agent Teams orchestration, TDD, code review, security audits, RLVR learning, MCP server, **Goal-driven autopilot**, **`/learning` diagnostics**, **`/save` + `/resume` single-shot handoff**, **`/go` → `/orchestrate` build-sequence hand-off**, **safety boost (machineId frontmatter, git-lock fail, 10m throttle)** |
