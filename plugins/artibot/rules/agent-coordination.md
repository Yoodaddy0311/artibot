...
- **Fable (10/30)**: 설계·검수 역할만 — orchestrator, architect, planner, code-reviewer, spec-reviewer, quality-reviewer, llm-architect, repo-benchmarker, investigator, auditor. `artibot.config.json#/agents/modelPolicy/fable/enabled=true` + `fable.allowlist` 10종 + 해당 `agents/<name>.md` frontmatter `model: fable`. 근거: 오너 9/1 정책 "구현·테스트 서브에이전트 = opus, 검수·설계 = fable"(Ontology queue.md).
- **Opus (20/30)**: 구현·마케팅 에이전트 전부. `high` 버킷이 `model: fable` 을 선언해도 allowlist 밖이면 게이트가 opus 로 강등한다(의도된 동작). `security-reviewer` 는 `FABLE_DENYLIST` 로 영구 opus(refusal 오탐 — 5.1 오탐률 미측정, 측정 전 해제 금지).
...
- **Sonnet / Haiku**: 정책 미사용.
- **되돌리기(단일 티어 opus)**: `fable.enabled=false` 한 줄 + 10개 frontmatter `model: opus`. `scripts/ci/validate-model-policy.js` 가 드리프트 게이트.
