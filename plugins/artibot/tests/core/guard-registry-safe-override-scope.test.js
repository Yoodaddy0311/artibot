...
const PARITY_MATRIX = Object.freeze([
  ...
  },
  {
    ...
    l2Id: null,
    status: 'owner-decision',
    note: '줄바꿈 경계가 L2 에서만 작동한다. 두 층의 정규식은 바이트 동일하고 옵션 런을 줄바꿈에서 끊지만, L1 은 guard-registry.js#checkDangerousCommand 가 normalizeCommand 변형도 대조하고 normalizeCommand 는 `/\\s+/g` 로 줄바꿈을 공백 하나로 접는다 — 패턴이 보기 전에 줄바꿈이 사라진다. 그래서 뒷줄의 -f 가 강제 플래그로 읽혀 이 안전한 2줄 스크립트가 L1 에서 차단된다(2026-09-11 executeChain 실측, CRLF 도 동일). 근본 해결 후보는 normalizeCommand 의 `/\\s+/g` → `/[^\\S\\n]+/g` 인데 BLOCKED_PATTERNS 38규칙 전부에 영향을 주므로 별도 줄기 + 오너 결정이다. 정규식을 느슨하게 해서 맞추지 말 것.',
  },
  {
