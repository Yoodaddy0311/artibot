...
// 움직이지 않아야** 한다. 코퍼스 전후표(20,920 문자열, level+matchedId 완전
// 동일 — 스냅샷 961,266 바이트가 양쪽 바이트 동일)는 그것을 코퍼스 안에서만
// 말한다 — 코퍼스에 없는 플래그 표기는 아무것도 증명하지 못한다. 그래서
// 코퍼스에 기대지 않는 **전수 열거**를 여기 둔다.
// (같은 날 틸드/$HOME 편집이 인용하는 1,967 문자열 코퍼스는 **등급 변화**를 잰
//  다른 수확분이다. 이 줄의 20,920 은 언어 보존 코퍼스다.)
//
...
// 알파벳 {d,D,f,F,x,9,_,-} **밖의** 글자를 클래스에서 더 빼는 변이(E·Z·c 제거,
// `[a-df-z]*d` 등)도 여기서는 불일치 0 이다(검수 실측 2026-09-23 12:1x KST, 변이
// 4종 전부 0). 그런 변이는 아래 소스 모양 핀(일부는 regex-scan 1-b 게이트)만 잡는다.
