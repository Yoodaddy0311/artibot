 ...
 *
 * ── 이 파일에 있었던 NUL 바이트 사고 (2026-08-16, 해소) ─────────────────────
 * 이 파일이 커밋 `cd56ebfe` 에 **NUL 바이트 2개를 품은 채** 들어갔다. 아래 stale
 * 판정 키의 **구분자 공백이 `0x00` 으로 기록**됐다 (offset 17591 · 17777):
 *
 *     `${r.file}<NUL>${r.tool}`   그리고   `${file}<NUL>${tool}`
 *
 * **왜 전건 그린이었나**: 양쪽이 *같은* 구분자를 썼으므로 `live.has()` 조회가
 * 일관되게 맞았다. 테스트는 19/19 통과했다.
 *
 * **그래도 왜 결함인가**: ① `git`·`grep` 이 이 파일을 **바이너리로 취급**해
 * 검색이 안 됐다(실제로 이 주석을 쓰던 중 `grep` 이 `Binary file matches` 만
 * 뱉어 막혔다). ② **fail-open 지뢰** — 두 줄 중 **한쪽만** 정상 공백으로 다시
 * 타이핑하면 키가 어긋나 **stale 항목 검출이 조용히 영영 안 돈다.** 이 게이트가
 * 막으려는 결함과 같은 계열이 게이트 자신 안에 있었던 셈이다.
 *
 * **범위 (다시 조사하지 마라)**: `git ls-files '*.js' '*.mjs' '*.md' '*.json'`
 * **1,546 파일 전수 스캔에서 NUL 을 가진 파일은 이것 하나뿐이었다** (리더 실측,
 * 2026-08-16). 수정 후 잔존 NUL 0, 바이트 수 25,469 불변, 19/19 유지.
 * **유입 단계는 특정하지 못했다** — 최초 `Write` / `eslint --fix` / 커밋
 * 파이프라인 중 어느 것인지 미확인이다.
 *
 ...
 *
 *  1. **`allowed-tools` 는 스폰된 서브에이전트에게 상속되지 않는다** (2026-08-16
 *     실측으로 확정 — 이 게이트를 쓸 당시에는 미확인이었고, 그래서 가정을
 *     하드코딩하지 않은 선택이 결과적으로 옳았다). 리더가 `repo-benchmarker` 를
 *     직접 스폰해 확인: 그 에이전트 정의에는 `WebFetch` 가 있고 `commands/repo.md`
 *     의 `allowed-tools` 에는 없는데, **스폰된 쪽은 `WebFetch`·`WebSearch` 를 둘 다
 *     보유했고 각각 1회 호출에 성공**했다. 즉 서브에이전트 도구셋을 정하는 것은
 *     `agents/<name>.md#tools` 이지 커맨드의 `allowed-tools` 가 아니다.
 *     **그래서 "위임 문맥 자동 면제"는 더더욱 틀린 설계다** — 상속이 없으므로
 *     캐리어의 선언은 오직 **캐리어 자신이 호출하는 것**에 대한 진술이고, 그것이
 *     바로 이 게이트가 재는 것이다.
 *     관측의 한계(지우지 말 것): 관측자는 `/team` 컨텍스트에서 스폰됐지 `/repo`
 *     컨텍스트가 아니다. 두 호출이 성공했다는 것은 "이 스폰 경로의 이 두 호출이
 *     안 걸렸다"까지만 증명한다 — allowlist 미등재 도메인 거동은 미측정이다.
 ...
 *  6. **위임된 도구는 이 게이트의 관할이 아니다.** 상속이 없다는 것이 확정됐으므로
 *     (위 "위임 예외" 1번), 캐리어가 선언하지 않은 도구라도 **서브에이전트 쪽
 *     `agents/<name>.md#tools` 로 실동작이 성립할 수 있다.** `commands/repo.md` 의
 *     `WebFetch`/`WebSearch` 언급이 정확히 그 사례이고, 그래서 **무해**다.
 *     이 게이트는 그 판단을 자동으로 내리지 않는다 — "위반"이 아니라 "**등록되지
 *     않은 참조**"를 실패로 삼고, 판단은 등록 시점에 사람이 근거와 함께 내린다.
 *     **에이전트 쪽 `tools:` 와의 교차 검증은 하지 않는다** — 어느 `Agent(...)`
 *     호출이 어느 에이전트를 띄우는지 본문에서 기계적으로 잇지 못하기 때문이다.
 *
