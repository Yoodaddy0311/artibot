 ...
 *   - **safe_copy_dir 은 3단 폴백이고 이 스위트는 그중 하나만 검사한다.**
 *     rsync → tar 파이프 → per-file cp 루프 순으로 내려가며, 실행 환경에 있는
 *     티어만 실제로 도는다. 이 머신 실측(2026-08-19, Win11/Git Bash): rsync 없음,
 *     tar 있음 → **tar 티어**. `bash -x` 추적으로 tar 분기 6회 실행 / cp 루프 진입
 *     0회를 확인했다. **cp 루프 티어는 여기서 한 번도 실행되지 않으므로 이 스위트가
 *     보증하지 않는다** — tar 없는 머신의 설치는 미검증이다.
 *     소요는 티어에 따라 갈린다: cp 루프 시절 190~330s → tar 티어 **24~42s**.
...
/**
 * 실측 이력 (전부 Windows 11 / Git Bash / rsync 없음).
 *
 * ── tar 티어 도입 이후 (2026-08-19~) ─────────────────────────────────────────
 *   - **24~42s** (42.1s 단독 / 24.5s 2파일 동시). safe_copy_dir 에 tar 파이프
 *     티어가 앞에 붙으면서 아래 cp 루프 시절 수치의 1/5~1/13 이 됐다.
 *
 * **그래도 이 상수를 24~42s 기준으로 낮추지 않는다.** tar 가 없거나 이 플래그를 거부하는
 * 머신은 여전히 cp 루프로 강등되고, 그때 소요는 아래 이력 그대로다. 상수는 **가장
 * 느린 티어**를 견뎌야 한다 — 빠른 티어에 맞춰 조이면 폴백이 실제로 발동한 바로 그
 * 머신에서만 빨개지고, 그건 진단이 가장 어려운 형태의 빨강이다.
 *
 * ── cp 루프 티어 시절 (2026-08-18~19, 이 스위트가 그때 잰 값) ────────────────
 ...
 *
 * cp 루프 티어에서는 같은 코드가 190s → 520s 로 2.7배 흔들렸다. 그래서 이 값은
 * "평균의 몇 배"가 아니라 **관측된 최댓값 기준**으로 잡는다 — 520s 대비 3.4배.
