...
// 파서·전개기는 형제 모듈에 있다. 이 파일은 **판정만** 들고 있다.
import {
  extractHookRequiredContexts,
  extractPushBranches,
  renderJobNames,
} from './workflow-branch-lockstep.js';
...
//     fail-closed 된다(아래 단언으로 고정). **block·flow 두 표기가 같은 판정을
//     내야 한다** — 한쪽만 fail-closed 면 다른 쪽 표기로 게이트를 우회할 수 있다.
...
describe('스캐너 자기검증 — matrix include 병합', () => {
  ...
  // 아래 두 건은 예약 키의 **표기법 간 판정 일치**를 못박는다. 파서는 block 형
  // 시퀀스만 읽는데, 예전 판은 flow 형 예약 키를 그냥 건너뛰어 block 과 flow 가
  // 서로 다른 판정을 냈다. exclude 쪽은 그 차이가 곧 fail-open 이었다.
  it('flow 형 `exclude:` 도 block 형과 같은 판정이다 (표기법으로 우회 불가)', () => {
    // 실측 2026-08-19(수정 전): 이 입력이 required context 인 "Validate (Node 22)"
    // 를 포함한 이름 3개를 그대로 냈다. exclude 가 그 조합을 지우는데도 게이트는
    // 초록이었고, 브랜치 보호는 영영 충족되지 않는 상태로 굳는다.
    const flow = renderJobNames(
      jobYaml('Validate (Node ${{ matrix.node-version }})', [
        '        node-version: [20, 22, 24]',
    ...
    expect(flow).toEqual([]);
    expect(flow).not.toContain('Validate (Node 22)');

    // 같은 의미의 block 표기와 판정이 일치해야 한다.
    const block = renderJobNames(
      jobYaml('Validate (Node ${{ matrix.node-version }})', [
        '        node-version: [20, 22, 24]',
        '        exclude:',
        '          - node-version: 22',
  ...
  it('flow 형 `include:` 도 fail-closed 다 (읽지 못하는 표기를 읽은 척하지 않는다)', () => {
    // 이쪽은 조합을 더하는 방향이라 fail-open 은 아니었지만(이름이 모자랄 뿐),
    // 파서가 못 읽는 표기를 조용히 무시하는 것은 같은 결함이다. 못 읽으면
    // 이름을 안 낸다 — block 형과 판정이 갈리지 않게.
    const names = renderJobNames(
      jobYaml('Validate (Node ${{ matrix.node-version }})', [
        '        os: [ubuntu-latest]',
        '        node-version: [20]',
        '        include: [{os: windows-latest, node-version: 22}]',
      ]),
    );
    expect(names).toEqual([]);
  });
