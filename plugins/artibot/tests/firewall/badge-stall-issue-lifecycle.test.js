 ...
 *
 * ── 분할 (2026-09-11) ────────────────────────────────────────────────────────
 *   이 파일은 1,033줄이었다. 배지 escalation 과 **같은 릴리스 잡**에 살던 두
 *   계약을 순수 이동으로 떼어냈다 — 테스트 이름과 단언 줄은 한 바이트도 안 바뀌었다.
 *     - `sync-readmes` 스텝 순서 계약 → `badge-stall-sync-order.test.js`
 *     - 힐링 대상 ↔ 커밋 대상 집합 lockstep → `badge-stall-landing-lockstep.test.js`
 *   공유 로더·절단기(`source`·`sliceStep`·`executableShell`)는 테스트가 아닌
 *   `badge-stall-yaml-tools.js` 로 갔다. 각 파일이 **못 보는 것**(rules §9)은
 *   그 파일 안에 그대로 따라갔다 — 여기 아래 목록은 이 파일의 6중 게이트 몫이다.
 *
