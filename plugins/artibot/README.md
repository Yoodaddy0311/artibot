...
[![Version](https://img.shields.io/badge/version-4.57.0-blue?style=flat-square)](./CHANGELOG.md)
...
> the themed statusline now auto-resolve both layouts (native install included) — a
> native-only install may just need `/theme` re-run after a major `/plugin` update.
> `/update` still assumes the flat layout and is being migrated to `${CLAUDE_PLUGIN_ROOT}`.
> Commands, agents, skills, and hooks themselves work identically on both paths.
...
| `updateCheck.enabled` | `true` (absent) | Session-start update check. Optional key — not shipped in the default config, add it to opt out; env `ARTIBOT_UPDATE_CHECK=0` wins over config. Off = no network call, no cache write |
...
| 5 | **Hooks** | 16개 이벤트에 연결된 자동화 파이프라인. 포맷팅, 검증, 추적, 외부 알림을 코드 변경 없이 처리 |
...
| `advisor-strategy` | Executor + Advisor 이원화 패턴 (단일 티어 편성이라 executor/advisor 모두 opus) |
