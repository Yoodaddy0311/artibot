 ...
 *
 * Compared after {@link identityOf}, so `artibot:code-reviewer` matches.
 ...
 */
function isReviewerStop(agentType) {
 ...
 */
function recordReviewFromStop(hookData, ids, projectRoot) {
  try {
    const { sessionId } = ids;
 ...
 */
function reviewLedgerColumn(summary) {
...
function handleStop(hookData, ids) {
  ...
  const missionId = resolveMissionId(hookData, sessionId);
  const projectRoot = payloadProjectRoot(hookData);
  // The tracked type is preferred for the same reason the spawn column below
  // prefers it: it is what the START of this epoch recorded, and a stop payload
  // that disagrees with it is describing a different spawn.
  const effectiveType = tracked?.agentType ?? agentType;
  // Reviewer stops only. Everything else skips the path entirely — see
  // `isReviewerStop` for why this is an allowlist.
  const review = isReviewerStop(effectiveType)
    ? recordReviewFromStop(
        hookData, { agentId, agentType: effectiveType, sessionId, missionId }, projectRoot,
      )
    : null;
  recordSpawn(hookData, projectRoot, {
    event: 'stop',
