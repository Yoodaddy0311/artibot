...
/**
 * Fixed strings that this bench uses but did not INVENT. Counted and reported,
...
/** Sandbox directory spellings generated by this process. See recordSandboxPaths(). */
const EMITTED_SANDBOX_PATHS = new Set();
 ...
 * row is written `C:\\Users\\…`, so a substring search for the native spelling
...
function recordSandboxPaths(dirs) {
  for (const dir of dirs) {
    EMITTED_SANDBOX_PATHS.add(dir);
    EMITTED_SANDBOX_PATHS.add(dir.split(path.sep).join('/'));
    EMITTED_SANDBOX_PATHS.add(path.basename(dir));
  }
}

/**
 * The exact values whose appearance in a real store is a leak.
 * @returns {string[]}
 */
function exactLeakMarkers() {
  return [...EMITTED_SESSION_IDS, ...EMITTED_SANDBOX_PATHS];
}
...
const PID_PROBE_SOURCE = [
  ...
  '  if (log) {',
  "    const script = path.basename(process.argv[1] || '(none)');",
 ...
 * `.artibot/runtime` at 2026-09-11T00:08 local and had
 * `.artibot/runtime/ledger.jsonl` (2744 bytes) at 00:22, written by the live
 * session's own hooks with nothing to do with this file.
 */
 ...
 *
 *   1. Bare `bench-` substring, 2026-09-11T00:33Z. The checkout is a worktree
 ...
 *
 * Both were the same mistake at different resolutions, so the fix is not a
 * third, cleverer pattern. The verdict now uses ONLY values this process
 * generated and recorded at generation time:
 *
 ...
 * it there, and no before/after arithmetic is needed to attribute a hit.
 *
 * The prefix markers survive as INFORMATIONAL counts. They are still worth
 * seeing — a stale `artibot-bench-` from an earlier run says something about
 * this tool's history in that store — but they never fail a run, because a
 * count that a passer-by can raise is not evidence about this process.
 *
 * @param {string} root
...
function snapshotOne(spec) {
  ...
  // there: a concurrent writer must not fail the run, but a bench fingerprint
...
function compareLeakScan(before, after) {
  ...
  // No before/after subtraction: an exact marker carries a random suffix minted
  // by this process, so its presence anywhere is attributable on its own and
  // its absence is the whole claim.
  return {
...
export function compareGuards(before, after) {
  return (before || []).map((prev, index) => {
    ...
    // consulted — a bench fingerprint in the store is a violation whether or not the
...
function parseArgs(argv) {
  const opts = { slot: 'all', n: 20, warmup: 2, json: false, guards: [], probe: true, help: false };
  let index = 0;
...
async function main() {
  ...
  const guards = compareGuards(before, after);
