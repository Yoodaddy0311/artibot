 ...
 *
 * NEWLINE CONTRACT (public API — re-exported from lib/core/index.js).
 * The returned string PRESERVES line breaks, because a bare newline is a shell
 * command separator and the BLOCKED_PATTERNS rules are written to respect that
 * boundary (see the `[^\S\n]` option runs in the git-branch rule and the
 * `[^\n]{0,192}` bound in the dd rule, blocked-patterns.js). Specifically:
 *   - a BACKSLASH followed by LF or CRLF is a line continuation — one command —
 *     so it is joined into a single space;
 *   - every remaining CRLF becomes a bare LF, which survives;
 *   - only intra-line whitespace runs (`[^\S\n]+`, which also folds a lone CR)
 *     collapse to one space.
 * Folding newlines into spaces instead — what this function did until
 * 2026-09-11 — glued adjacent lines together, so L1 read a two-line script as
 * one command and blocked the safe `git branch -d old` + `echo -f done` at
 * PreToolUse while L2 (lib/autopilot/safety.js) graded it safe.
 * Joining the backslash continuations FIRST is load-bearing: without that step
 * the escape survives the fold (the `\\(.)` unescape above never crosses a
 * newline) and a genuinely dangerous continued command such as
 * `rm -rf \` + LF + `foo/` would newly pass.
 * The whole rule set was measured through executeChain, cell by cell, on
 * 2026-09-11: NO regression — no single dangerous shell command that used to be
 * blocked now passes. The relaxations are all cases where a bare newline really
 * does split the input into separate shell commands; the rest of the deltas are
 * strengthenings, continuation forms that used to walk through.
 * Counts are deliberately NOT repeated here — BLOCKED_PATTERNS changes and any
 * number copied into this comment rots silently (it already did once, when the
 * fork-bomb rule was repaired mid-flight). The per-cell table, its denominator
 * and its measurement timestamp live in
 * `docs/investigations/guard-normalize-20260911.md`, which is regenerated by
 * re-running the measurement script.
 * Callers that need a single-line string must fold the newlines themselves.
 *
 * WINDOW BOUND (ReDoS, 2026-09-14). The `$(…)` and `${…}` unwrappers used to
 * carry an unbounded `[^)]*` / `[^}]*`. An opener that never closes makes that
 * quadratic: the engine restarts at every opener and scans to end of input
 * before failing, so an input of N bytes made of `$(` has N/2 openers times an
 * N-byte scan. MEASURED (median of 3, node v24.15.0, Windows 11, a `$(` run
 * through this function alone):
 *     20,480B  155.81ms · 40,962B  580.42ms · 122,880B  5917.25ms
 *     growth 122,880/20,480 = 37.05   (linear 6.0, quadratic 36.0)
 * `${` runs behaved the same (growth 14.22 over the 4x interval 10,240 ->
 * 40,962, quadratic expectation 16). The other openers here were already
 * linear over that interval and are untouched: backtick 1.06, `"` 1.07, `'`
 * 1.10, `$` 1.08, `\` 1.23. This function sits on the PreToolUse path and is
 * re-exported from lib/core/index.js, and the W7 static scan only reads the
 * BLOCKED_PATTERNS / DANGEROUS_PATTERNS catalogues, so it could not see these
 * two. Bounded to `{0,192}` — the 4.60.0 convention shared with the L2
 * dd/curl/wget/git-push rules.
 *
 * WHAT THE BOUND GIVES UP, precisely. A substitution or expansion body LONGER
 * than 192 characters is no longer unwrapped, so that one span stays spelled
 * `$(…)` / `${…}` in the NORMALIZED variant. Nothing is deleted and nothing is
 * hidden: `checkDangerousCommand` tests the RAW variant too, and the raw
 * variant still contains that span byte-identical, so a dangerous command
 * inside an over-long substitution is still matched there. The loss is
 * therefore confined to rules that could only ever have matched the UNWRAPPED
 * spelling of a >192-character body — direction is toward fewer matches on the
 * normalized variant only, never toward more. Pinned by the bound-edge tests in
 * tests/core/blocked-patterns.test.js (`normalizeCommand substitution window
 * bound`): 192 unwraps, 193 does not, and the dangerous command inside a
 * >192-character substitution still blocks through executeChain.
 *
 * @param {string} raw
 * @returns {string} normalized command, with line breaks intact
 */
export function normalizeCommand(raw) {
  ...
  cmd = cmd.replace(/`([^`]*)`/g, '$1');
  cmd = cmd.replace(/\$\(([^)]{0,192})\)/g, '$1');
  cmd = cmd.replace(/\$?'([^']*)'/g, '$1');
  cmd = cmd.replace(/"([^"]*)"/g, '$1');
  cmd = cmd.replace(/\\(.)/g, '$1');
  cmd = cmd.replace(/\$\{[^}]{0,192}\}/g, ' ');
