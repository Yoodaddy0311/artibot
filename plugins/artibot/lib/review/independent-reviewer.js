...
// ---------------------------------------------------------------------------
// Ledger writers — review.completed / review.claim_audit
// ---------------------------------------------------------------------------

/**
 * ── Why the writer lives in the contract module ─────────────────────────────
 * The parsers decide whether an answer is admissible; these functions turn an
 * admissible answer into the INPUT of a ledger line. Keeping them here is what
 * makes "inadmissible ⇒ no line" a single decision rather than two that could
 * drift: a caller cannot reach the builder without going through the parse
 * result, and the builder refuses anything whose `ok` is not `true`.
 *
 * ── Still no I/O ───────────────────────────────────────────────────────────
 * L2 may not import `lib/runtime/` (L5), and this module must stay pure
 * anyway. The append and the already-written-keys lookup arrive as PORTS, so
 * the ledger writer is the caller's dependency, not this module's. The ports
 * mirror `lib/verification/unified-verifier.js#recordVerification`, which
 * solved the same problem for `verify.completed`.
 *
