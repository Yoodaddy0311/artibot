...
import { ensureDir, exists, listFiles } from '../core/file.js';
import fs from 'node:fs/promises';
import { syncTodo as syncPlanState } from './plan-state.js';
...
// ---------------------------------------------------------------------------
// TODO / plan state (implementation: ./plan-state.js)
// ---------------------------------------------------------------------------

/**
 * Parse a plan markdown into checkbox tasks and persist `.plan-state.json`
 * alongside the resolved plan file. Full contract (fail-closed rules, merge
 * semantics) is documented on the implementation in `./plan-state.js`.
 *
 * Deliberately a declared `export async function`, not a bare
 * `export { syncTodo } from './plan-state.js'`: the async/await doc-parity
 * gate (`tests/firewall/doc-async-await-parity.test.js#collectAsyncExports`)
 * matches `^export async function` declarations and requires `syncTodo` to be
 * declared in this file, which `commands/*.md` dynamic-import call sites load.
 *
 * @param {object} args - See `plan-state.js#syncTodo` for the full shape.
 * @returns {Promise<{ ok: boolean, stateFile?: string,
 *   progress?: { total: number, completed: number, percentage: number },
 *   error?: string }>}
 */
export async function syncTodo(args) {
  return syncPlanState(args);
}
