...
| 타깃 | `#symbol` **권장**, `:NN` 허용. 범위 `:NN-MM` 와 콤마 `:NN,MM` 지원 | `install.sh#acquire_install_lock` |
...
- **skip (검사 대상 아님, 사유별 계수)**: 플레이스홀더 리터럴 · 리포 밖 prefix · 베어 베이스네임 · 미등록 확장자 · 미등록 루트 세그먼트. 사유는 열거형 allowlist — 미분류 skip 은 위반이다.
...
실측 규모(2026-08-17 census): 베어 베이스네임 skip **43건** — 여기엔 `artibot.config.json#…` 같은
해소 가치가 있는 루트 파일 인용도 포함된다(회수는 v2 후보). 위치 표기 없는 **맨경로 언급**은 이
게이트의 문법 밖이며, 적대 검증의 광의 census 는 그 층에서 매달린 참조 ~57건(대부분 GRPO/voyager
철거로 삭제된 파일을 가리키는 역사 기록물)을 관측했다 — 별도 v2 후보로 기록한다.
