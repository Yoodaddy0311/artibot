...
const { readPlanCompletion } = await import(toFileUrl(path.join(root, 'lib/git/limb-completion.js'))); const { readLaneOpsState } = await import(toFileUrl(path.join(root, 'lib/supervisor/lane-monitor.js')));
