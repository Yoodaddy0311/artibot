---
description: (Artibot) Cross-session multi-worktree split — 파일 소유권이 겹치지 않는 줄기를 창 N개(상한 8)로 병렬 진행하고, 완료를 git 트레일러로 판독한다. 서브커맨드는 plan·open·status·dispatch·run·integrate·handoff·resume, 운용 스크립트는 land·watch·probe·worktree-setup·restore-blob·suspend
...
7. `plan.json`(`run.json` 과 같은 디렉터리 — `path.join('<parentRoot>', '.artibot', 'split', 'plan.json')`) 에 `{ runId, sid, parentRoot, parentSession, repoShort, base: git rev-parse HEAD, createdAt, limbs:[{ limb, worktreeName, worktreePath, branch, taskIds, affectedPaths, forkPoint? }], plan }` 를 쓴다 — `dispatch`·`resume` 의 입력 형태(`lib/git/split-dispatch.js#resolveDispatch` 가 `{ runId, base, limbs[].worktreePath/branch }` 를 읽는다). `forkPoint` 는 plan 이 쓰지 않는다 — `dispatch <limb>` 스크립트가 worktree 에서 `git merge-base <ref> HEAD`(ref 는 `master`→`main`→`origin/master`→`origin/main` 첫 성공 — 로컬 우선, worktree 가 로컬 HEAD 에서 분기하므로; 창이 이미 커밋을 쌓았어도 분기점, `rev-parse HEAD` 는 작업 팁이라 틀리다) 를 **없을 때만** 채운다(`lib/git/split-run-file.js#updatePlanJson`, 재발행 멱등; 네 ref 다 없으면 미기록 + 사유). `base` 는 plan 시점 SHA 라 그 뒤 master 가 전진하면 worktree 분기점과 달라지고 `land`·완료 판독이 남의 파일을 줄기 변경으로 읽는다(2026-09-14 실측 docs 6파일, 리더 수동 이동) — 읽는 쪽은 전부 `limbs[].forkPoint || plan.base`(빈 문자열 = 미기록). 두 파일 모두 `schema_version` 키(현재 1)를 쓰기 시 없을 때만 스탬프한다. 그 디렉터리는 루트 `.gitignore:86` 으로 ignore 된다(2026-08-26 추가) — 로컬 런 상태이므로 커밋 경로에 넣지 마라. `recordPhaseEnd(runId, 'PLAN')`. **`parentSession` 은 선택이 아니다** — `scripts/split/dispatch.mjs` 는 `--parent` 또는 `plan.json.parentSession` 을 요구하고 없으면 dry-run 부터 실패한다(2026-09-04 실발생 1회). 값은 이 창의 세션 이름이다.
...
판단은 코드(`lib/git/split-dispatch.js#resolveDispatch`, 순수 함수 — 같은 입력 → 같은 출력), 행동은 리더(`messages[]` 를 그대로 `SendMessage`). 진실원은 git 과 파일시스템, 메시지는 최적화다. `recordPhaseStart(runId, 'DISPATCH')`. **줄기 안의 팬아웃은 리더가 설계한다** — 브리프 §1 에 줄기별 분해 권장안(하위 묶음 2~5개 · 각 묶음의 소유 파일 · 검수 배치)을 넣는다. 창은 리더이지 구현자가 아니다(창 쪽 규약은 `templates/split/PROMPT-TEMPLATE.md` §줄기 내부 팬아웃, 관측점은 `<git-common-dir>/artibot/spawns.ndjson` — 전 창이 공유하는 한 파일이고 창별 분리는 `sessionId` 로 한다, `fanout-probe`). 2026-09-04 실측: 템플릿에 팬아웃 지시가 없어 4창 중 3창이 스폰 0건으로 시작했다(gotchas #16).

0. **줄기 하나 배정(스크립트, 2026-09-02 — 프롬프트 전문 붙여넣기 금지)**: `node <pluginRoot>/scripts/split/dispatch.mjs <limb> [--window <세션>] [--dry-run] [--json]` 이 부모 브리프를 worktree 로 원자 복사하고(동반 파일 allowlist `split-brief.js#SIBLING_FILES` = `leader-addendum.md` 도 있으면 같이, 글롭 아님 — `brief-draft.md` 는 안 간다) `prompt.md` 를 템플릿에서 렌더한다(`lib/git/split-brief.js#materializeLimb` · `renderPrompt` — 필수 절 없음·미해결 `{PLACEHOLDER}` 는 refuse; `{REPORT_CONTRACT}` 는 이 문서의 `[보고 계약]` 펜스 그대로). 포인터는 brief.md **와 prompt.md** 둘 다 가리킨다(`split-dispatch.js#buildLimbMessage` 의 `promptPath`, 2026-09-14 창 3 이 prompt.md 를 못 받아 리더가 보충 발송한 결함). dry-run 이 아니면 두 기록을 더 한다: ① `lanes[limb] = { state: 'active', since, window }` 를 `scripts/split/lane-state.mjs#setLaneState` → `lib/topology/split-state.js#writeWorkerState` 경유로(writer 는 이 체인 하나, 어휘는 `LANE_OPS_STATES` allowlist — 손으로 `dispatched` 를 쓰면 `readLaneOpsState` 가 null 이라 선제외·watch·probe 가 전부 unknown, 2026-09-14 8/8 실측; 출력 `laneState.warning` 이 그런 기존 값을 알린다) ② plan.json `limbs[].forkPoint` 가 없으면 worktree 의 `merge-base <master|main|origin/*> HEAD` 로 채운다(§plan 7). 출력의 **`pointer` 1줄만** 리더가 `SendMessage(to, pointer)` 한다 — 스크립트는 보내지 않는다. 절차·플래그·근거(Ontology 9회 2.5KB 복제 전송)는 `skills/split/references/operations.md` §dispatch. 전체 줄기를 한 번에 판정·발송하는 절차는 아래 1~5.
...
const { readLimbCompletion } = await import(toFileUrl(path.join(root, 'lib/git/limb-completion.js'))); const { readLaneOpsState } = await import(toFileUrl(path.join(root, 'lib/supervisor/lane-monitor.js')));
...
const runJsonPath = path.join('<parentRoot>', '.artibot', 'split', 'run.json'); /* plan.json 과 같은 디렉터리 */ const runJson = fs.existsSync(runJsonPath) ? JSON.parse(fs.readFileSync(runJsonPath, 'utf-8')) : {}; /* run.json 부재(단독 dispatch) = lanes 없음 = 선제외 0, throw 아님 */ const excludeLimbs = plan.limbs.map((l) => l.limb).filter((l) => readLaneOpsState(runJson, l) === 'done').concat([/* 리더 명시 목록 */]); const opsStateUnknown = plan.limbs.map((l) => l.limb).filter((l) => runJson.lanes?.[l] !== undefined && readLaneOpsState(runJson, l) === null); /* F07: allowlist 밖 어휘(예 `dispatched`)는 선제외 0건으로 처리되고 여기 경고 필드로만 보인다 — `lane-state.mjs <limb> active|done` 으로 고친다 */ const decision = resolveDispatch({ plan, worktrees: listWorktrees(process.cwd()), sessions, messaging: messagingFromEnv({ listAgentsAvailable: true }), excludeLimbs }); // 선제외 진실원 = `run.json.lanes[limb].state === 'done'`(`readLaneOpsState` 경유 — `lane-state.mjs` 가 쓴 것만, allowlist 밖 어휘는 unknown) ∪ 리더 명시 목록. 제외 줄기는 판정에서 빠지되 `excluded[]`·`limbs[].excluded` 로 보인다(숨기지 않는다)
const done = new Set(plan.limbs.filter((l) => readLimbCompletion({ cwd: process.cwd(), branch: l.branch, base: l.forkPoint || plan.base }).complete).map((l) => l.limb)); // 줄기별 base = forkPoint || plan.base (§plan 7 — 빈 문자열은 미기록, `forkPointForLimb` 와 같은 판정)
process.stdout.write(JSON.stringify({ ...decision, opsStateUnknown, messages: decision.messages.filter((m) => !done.has(m.limb)) }, null, 2));
```
...
5. **보고**: `ready` 면 "줄기 N개 중 M개 전송(이미 완료 K개 제외), {측정시각}" + `opsStateUnknown` 이 비어 있지 않으면 그 줄기 목록과 "`lane-state.mjs <limb> active` 로 정정" 안내. 상태 파일은 **lane-state 형태로만** 쓴다 — 0단계 스크립트가 `lanes[limb].state='active'` 를 allowlist 어휘로 기록하고, 이 3~5단계는 그 외 어떤 키도 손으로 쓰지 않는다(`dispatched`·`dispatchedAt` 같은 사제 키 금지 — 2026-09-14 실측 원인). `recordPhaseEnd(runId, 'DISPATCH', { message })` (거부·불가도 닫는다).
...
`node <pluginRoot>/scripts/split/<x>.mjs`(`<pluginRoot>` = `CLAUDE_PLUGIN_ROOT` 또는 `~/.claude/plugins/cache/artibot/artibot/<최신>`), 전부 부모 루트를 cwd 로. 어느 것도 메시지를 보내거나 push·merge 하지 않는다. `land <limb> [--base <ref>]` (base 우선순위 `--base` > `plan.limbs[].forkPoint` > `plan.base` — 표 아래 `base` 정보 절 1줄이 어느 것을 썼는지·plan.base 와 다르면 몇 commits 인지·forkPoint 미기록이면 `git merge-base master <branch>` 참고값과 `--base` 안내를 낸다(정보 절이지 checks 행이 아니다 — 7행 핀 유지; 리베이스된 줄기·라이브 ref 의존은 못 본다, 문구만 낸다)) = 트레일러(first-parent) · allowlist 소유권 diff · 바이너리 0 · 금지 인용 0 · **lint**(변경된 `.js`/`.mjs` 한정 `eslint --max-warnings=0` 을 **줄기 worktree 의 `plugins/artibot` 을 cwd 로** 실행 — `plan.json` `limbs[].worktreePath` 로 찾고 그 HEAD 가 줄기 브랜치인지·린트 대상과 겹치는 미커밋 변경이 없는지 검증한다(2026-09-10 #G14 수리); 변경 0건이면 `SKIP`, worktreePath 미기재·HEAD 불일치·미커밋 겹침·eslint 부재·실행 실패는 각각 **다른 문장**의 `UNSUPPORTED`(앞 12자로 구분, fallback 없음) — PASS 로 넘기지 않는다. 2026-09-04: land 6/6 PASS 인데 eslint 는 오류 3·경고 2 였고 CI 는 `eslint . --max-warnings=0` 이라 배치가 빨개질 상태였다, gotchas #25) · **pin tests**(정보 절, 실행하지 않고 exit code 에도 안 들어간다 — 표적 스위트 결정은 리더): 변경 파일 중 설정 파일(`hooks.json`·`plugin.json`·`dispatch-table.json`·`ledger-events.allowlist.json`·`artibot.config.json`·`schemas/*.json`)의 basename 을 리터럴 인용하는 러너 루트 `tests/**/*.test.{js,mjs}` 목록(`scripts/split/land.mjs#pinTestsFor`, 분모 병기) — Wave 8 p4 에서 `hooks/hooks.json` 변경의 핀 `tests/hooks-schema-shape.test.js` 가 allowlist·표적 밖이라 not-green 이었다. 못 보는 것: 변수로 조립한 파일명·디렉터리 글롭 로더·`.test.js` 가 아닌 스냅샷(`hooks-schema-fingerprint.txt`)·줄기 자신이 추가한 테스트 · merge-tree 드라이런 · base 뒤처짐(`lib/git/limb-landing-check.js#checkLimbLanding`) → `PASS`(exit 0, **승인 아님**) / `FAIL`(빨간 행 `detail` 을 줄기 창에 `SendMessage`) / `UNSUPPORTED`(git < 2.38 → 직렬 강등); 줄기가 main 을 merge 했으면 살아 있는 ref 를 `--base` 로. `watch --parent <root>` = 줄기당 ops state(`run.json.lanes[limb].state`) · supervisor · 트레일러 · 마지막 커밋 · heartbeat · health(`lib/supervisor/lane-monitor.js#assessLane`, 결손은 `unknown`) + 측정 고지 raw — 부작용은 `runtime/split/{runId}.state.json` 캐시 1건, 폴링 루프 금지는 그대로. `fanout-probe --parent <root>` = `active` 줄기만 SOLO 경보, 미지 창·상태는 `(state unknown)` 로 **경보**(침묵 쪽으로 실패하지 않음). `worktree-setup <wt> --limb <limb>` = junction·`.env.local`·레인별 `lane.env`(e2e DB 분리), 멱등, `--teardown` 은 reparse point 만이며 창을 닫을 때 `git worktree remove` 보다 먼저 돈다(순서 정본은 operations.md worktree-setup 절). `restore-blob <f...>` = `git cat-file -p` 바이트 복원(autocrlf 가 지문을 깨는 `git checkout --` 대체). `suspend` / `resume-notices` = 재부팅 5단계 정지·재개 통지 생성 + `run.json.suspend`(회신 sha 는 리더가 git 으로 직접 확인).
