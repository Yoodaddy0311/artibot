---
description: (Artibot) 일일 회고 리포트 - 오늘의 작업/결정/다음 단계를 구조화된 대시보드로 출력 (alias of /daily)
...
Daily retrospective report. Collects git activity, quality metrics, task status, and worklog entries to produce a structured review dashboard. Functionally identical to `/daily` — both run the same workflow.

Also routed from: `/daily`, natural language "회고", "일일 보고", "오늘 작업", "오늘 뭐 했지"
