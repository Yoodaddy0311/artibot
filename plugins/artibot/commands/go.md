...
allowed-tools: [Read, Glob, Grep, Bash, Write, Task, TaskCreate, Skill]
lifecycle: genesis
---
...
**경로인 경우** (`commands/load.md:27-40` 패턴) — 구조와 프레임워크를 감지한다:
...
INTAKE 결과 → `{ domain, concepts[], stack, constraints }` 구조화 객체 생성.

## Phase 2 — BLUEPRINT: 6종 문서 생성 (MVP 핵심)

INTAKE 결과를 바탕으로 모델이 도메인을 추론해 구조화 객체를 만든 뒤 각 렌더러를 호출한다.
