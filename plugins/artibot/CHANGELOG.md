...
- **dispatch-base-forkpoint** (`f8a35282`, 배치 11, XS 부채): `dispatch.mjs` 가 forkPoint 를 **렌더 전에** 해석해 `{BASE}` 와 포인터가 `plan.base` 대신 기록된 분기점을 싣는다(`materializeLimb` 에 optional `forkPoint` 패스스루). 잔여: `resolveDispatch` 의 9필드 투영이 forkPoint 를 버려 일괄 발송 경로 포인터는 여전히 plan.base(Wave 11 XS). 창 재사용(ExitWorktree keep → EnterWorktree)으로 터미널 재개설 0회. 리더 실측 시각은 각 착지 결과 JSON(`run.json.landings`).

- **scorecard-absent-contract** (`b2ac11b9`, 배치 1 `a38a5ade`): routing 두 행의 `absent` 계약 위반 해소(A′ — absent:0 + `routing.tier_comparability` 행 신설), KNOWN DEFECT → 회귀. §4 ② 비율 정의 불변(4원장 fold diff 0, 라이브 167/167/20 = 11.98% @05:20Z).
- **nl-activation-report** (`92aa2f71`, 배치 2 `c9ad9819`): `scripts/evals/nl-activation-report.mjs` 읽기 전용 Shadow 계측기(SH-03) — 축 3개, 라이브 activation 2축 0/0 → null, `mission.deferral-rate` 251/274 = 0.916(@05:42Z). 분자 writer 는 리포 전역 0(오너 결정 대기).
- **economics-coverage** (`ebd6bc8b`, 배치 3 `d480c026`): 1부 `lib/replay/session-coverage.js#foldSessionCoverage` + CLI(Observe ④ 분모, 라이브 ended 0 → coverage null 이 정답) · 2부 **`priceReceipts` 기본 true**(행동 변화 — `usage.receipt.cost.total` 이 숫자가 된다, 방화벽 핀 갱신 같은 커밋; `=== false` 만 opt-out).
- **verify-numerator-rate** (`399c9ff2`, 배치 4 `c732eaa9`): 서브번들 A 만 — `lib/verification/verify-rate.js` + `scripts/ledger/verify-rate.mjs`(판독 전용). 분류 단위는 (session_id, verification_id) 쌍(브리프 정정). 라이브 `verify.completed` 는 설치본 `schemas/` 수정 뒤 05:21Z 부터 쌓이기 시작(13 id/6세션 @05:4xZ → 112줄 @08:44Z, self-report 0).
- **split-ops-remediation** (`22cc4323`, 배치 4): F06(pointer 에 prompt.md 경로) · F07(lane writer 단일화 + allowlist 밖 state 경고) · F08(`%(trailers:key=Split-Limb,valueonly)`) · `plan.limbs[].forkPoint`(첫 dispatch 때 `merge-base master HEAD`, land 는 `--base > forkPoint > plan.base`) · `materializeLimb` SIBLING_FILES · `schema_version`. 같은 웨이브의 롤링 4줄기가 이 코드로 dispatch·land 됐다(forkPoint 자동 적용 실측).
- **routing-single-decision** (`743ad7bb`, 배치 4): F04 기록 전용(`decision-events` `data.mode` 1필드, 양 System 에서 `buildWorkflowPlan` 계산·기록, `team.followWorkflowPlan` 예약 키 — 소비자 0, config 무접촉) · F05 effort identity(`sessionId+promptId`, `current-effort.json` 병행 기록, `expiresAt` 만료 게이트, GC 보존 32·TTL 10분). `auto-team-trigger` 는 분모 제외(판정 주석+테스트). R1 additionalContext 바이트 21/21 동일.
- **guard-l2-followups** (`39eab7a0`, 배치 5 `f43ce513`): L2 `rm-rf-root` 후보 C(터미네이터·`~`/`$HOME`·따옴표 타깃 — 래퍼 10형 miss 0, 잔여 `${HOME}`·`~user`) · `sql-delete-no-where` S2m(`[^;]{0,192}` — 단일행 WHERE 오탐 8/8 해소, 옛 규칙 2차식 36.8× → 선형) · 정적 스캐너 `tests/helpers/regex-scan.js` 로 단일화(예외 등록처 1, 분모 95=39+27+29). PARITY 57행. L2 규칙 수는 27(문서 "26" 은 기존 드리프트).
- **autopilot-phase-transition** (`a88eb5e7`, 배치 6 `e10ebabe`): **F01** 결과 보존(`resultHead/resultRef/integrationTarget`, integrationTarget 부재 = 삭제 금지, pruneOrphans 보존 목록) · **F02** durable phase 전이(`nextTarget` 순수함수 추출 → engine.js 순증 −10, `pendingPhase`, attempt journal, session v3 + `.bak`, v2 lazy reconcile). 프로브 AP-01/02/05 false(base 5/5 true). 행동 변화 (a) 강제 abort 가 dirty worktree·미통합 브랜치를 버리지 않음 (b) goal 일시정지 뒤 resume 이 EVALUATE 로 (c) 반복 중 phase 표시 EVALUATE (d) 세션 스키마 2→3(로드 시 메모리 마이그레이션, `.bak` 1회) (e)(f) cleanup 텔레메트리 신규 (g) 락 충돌 pause → INTAKE 재개 (h) recordSecretLeak 이 pendingPhase 기록. **4.62.0 노트 ③ 의 F01 임시 경고는 이 착지로 해제.** 거짓 그린 실사례: 8.3 단축 경로(`HEECHA~1`) vs porcelain realpath 불일치를 테스트가 early-return 으로 못 봄 → fail-closed `branch-unresolvable` + 8.3 회귀 케이스(`it.skipIf`).
- **ob17-models-current** (`a73eab24`, 코드 0 — 착지 생략): `models.current` 의 정본 정의는 세션 스코프 incumbent(`route-receipt.schema.json:117-120`)인데 PreToolUse ctx 에 원천이 없다 → 1줄로 닫히지 않음. 라이브 null 200/200, `hysteresis:minimum-residency` 가 incumbent 부재를 "미충족" 으로 판정해 residency 100% 가 구조적 산물(정찰 `ob17-switch-reasons`). 오너 결정 D1(incumbent 원천)·D2(`residency-unknown` 사유) 대기.
- **batch-landing-push-classify / F09** (`4f4e9c5f`, 배치 8 `25d3a4b0`): `landBatch` ⑦ ff push 거절을 `classifyFfRefusal` 로 — 원격 tip 재읽기 뒤 같으면 `push-failed`(stderr 원문·보존 batch SHA), 다르면 `moved`. RED 실증: 고정 tip 거절에서 build/CI/push 2·2·2·needs-human → 1·1·1·push-failed.
- **routebench-scenarios-live** (`9ae3b89d`, 배치 9): 오너 결정 E3=B — 라이브 시나리오 4행 + 스크럽 코퍼스 4파일(152행, N=185 @06:15Z) + 러너 `fixture-invalid` 소비 게이트, 33쌍 22 scored/11 refused. 첫 착지 CI RED(Windows autocrlf 체크아웃 → CR 이 'non-ascii' 규칙에 걸림) → `.gitattributes` `corpus/*.jsonl -text` 로 수정(규칙 무완화).
- **autopilot-budget-units / F03+F10** (`666683cb`, 배치 10): **예산이 토큰 단위로 실제 집행된다** — 구 코드는 `options.budget`(기본 2,000,000 토큰)을 USD 로 비교해 임계가 절대 발화하지 않았다. `budgetTokens` 정본 + `budget` 미러(1릴리스) + 선택 `budgetUsd`, `normalizeBudget` 단일 읽기, 단위별 `thresholdsFired {tokens,usd}`, `receiptId` 중복 1회 계수, 새 pause 사유 `budget-exceeded`(used ≥ limit), 미측정 사용량은 `unknown` + `budget-usage-unknown` 경고 1회/세션(오너 결정 E9 — pause 없음), `budget:0`·비수치 값은 no-budget 이 아니라 기본 2M(fail-closed). **F10**: `--no-team` 이 실행 지시에서도 단독(`execution:'solo'`, `teamHint.parallel:false`, Agent 스폰 0) — ADR-003 type 어휘 무변경. 프로브 AP-03·AP-04 false(→ 5/5 false). 라이브 영향: 부모 store 2,668 세션 중 pause 로 뒤집히는 세션 **0**(무예산 2,662 는 테스트 잔재 추론, compat 예산 6 은 전부 usage unknown → 경고 1회). fable R1 REJECT(문자열 `--budget` 이 한도 소멸) → 경계 강제로 수리 후 APPROVE.
- **plan-md-emitter** (`68b9b19a`, 배치 9): `lib/planning/plan-artifact.js`(plan.md 직렬화기, gates.js 원문 대조) + PreToolUse Write|Edit 형제 `_plan-observe-record.js`(plan.md 쓰기 → `plan.revision` +1 → `plan.revised{revision, mode:'plan'}`) + `tasks.js#planRevisionMutator`. 킬스위치 false 출하 유지 → 라이브 파일 생성 0 이 정답, 원장 append 는 게이트와 무관. README 훅 스크립트 카운트 72→73 동반.

**리더 실측 요약**: 창 재사용 프로토콜(ExitWorktree keep → EnterWorktree name)은 CLI `--worktree` 세션에서도 동작(6회 실측), `worktree.baseRef` 미설정이라 origin/master 최신에서 분기. 리더 세션은 유휴 중 창 메시지로 깨지 않아 `/loop` 15분 틱으로 큐를 드레인했다(1시간 공백 1회 발생). plan.json allowlist 에 테스트 파일 누락으로 land ownership FAIL 3회(리더 누락, 보충 후 PASS). Wave 11 정찰 초안 9건 산출(`.artibot/split/*/brief-draft.md`).
...
- **avoided-switch-fold** (`c158a718`, p1): `lib/scorecard/routing-scorecard.js` 에 Avoided Switch fold(추천≠정책 스폰 중 pin 사유가 있는 회피) + `lib/scorecard/index.js` export + `tests/scorecard/routing-scorecard.test.js` 신설 (4 files +493/−5). 라이브 첫 값 **`route.selected` 102행 중 12 = 11.8%**(2026-09-14 09:10, dedupe 후). **못 보는 것**: `data.source` 가 133/133 shadow 이고 프로덕션 줄 0 이라 `routing-scorecard.js` 헤더 #6 서술과 불일치하며, `residency:unavailable` 도 133/133 이라 "residency 100%" 는 미보정 상수의 그림자다(추론 — OB-17 `actionsSinceSwitch` 선행). pin 0 의 원인은 `route-observe-pre.js:259-276` 이 `currentTier` 를 넘기지 않는 것. `metric()` 의 absent 계약과 두 행의 서로소 absent 가 충돌해 models 없는 영수증이 과반이면 throw 한다(KNOWN DEFECT 핀 `:504-517`, 계약 A/A′ 는 오너 결정 후보). V5 OB-21 done, §4 ② 첫 수치.
- **routebench-b4-agenttype** (`1fbe6961`, p1): `scripts/bench/routebench.mjs` B4 resolver 가 `routeModel` 에 `input:{agentType}` 을 공급하도록 바꾸고 `baselines.{json,schema.json}` 을 맞춤 (5 files +260/−25). 오너 결정 ③(2026-09-14 00:2x "공급") 이행이며 lib 무변경. `baselines_sha256` 이 바뀌므로 `_benchmarks/routing/` 의 이전 로컬 리포트와는 비교 불가 라벨이 붙는다. **미확인·후속**: B3 정적 표의 `security-reviewer`→fable 괴리는 그대로(denylist 는 `resolveModel` 전용) · 봉투에 `policy_source.config_path` 미기록 · 러너 테스트의 로컬 BASELINES 사본에 `schema_version` 1 이 의도적으로 잔존. V5 SH-25 후속 done(SH-25 자체는 Wave 8 에서 done).
- **split-ops-dispatch-land** (`2fc59540`, p1): `lib/git/split-dispatch.js` 가 착지·정리된 줄기를 dispatch 판정에서 선제외하고, `scripts/split/land.mjs` 가 변경된 설정 파일의 핀 테스트를 나열한다 + `tests/split/land-pin-tests.test.js` 신설 · `commands/split.md` §dispatch·§land 문구 (5 files +558/−14). gotcha 91·92 대응. **못 보는 것**: `excludeLimbs` 는 `split.md` 절차 코드에만 배선돼 있고 `dispatch.mjs` 는 `resolveDispatch` 를 부르지 않는다 · `pinTestsFor` 는 줄기 자신이 추가한 테스트를 스캔하지 못한다 · `run.json` 의 `dispatched`/`landed` 를 쓰는 주체는 여전히 리더 수기(lib·scripts 0) 라 allowlist 어휘로 써야 선제외·watch 가 읽는다. V5 §3 부채 done.
- **session-end-vocab** (`5b2f8b5e`, p2): `schemas/ledger-events.allowlist.json` 에 `session.ended` 1항목 추가(총 **39 항목**, `node -e` 로 `events` 키 수 실측) + `scripts/hooks/session-end.js#recordSessionEnded` 가 `usage.receipt` 스테이지 성패와 무관하게 1행을 쓴다 (3 files +380/−5). 이름은 allowlist 의 `phase.ended`·`wallclock.ended` 과거분사 규약을 따른 리더 결정 L-2. 이것이 Observe 종료 판정 축 ④ 의 **분모**다. **못 보는 것**: 설치본이 갱신되기 전까지 라이브 발화 0 · `session-end.js` 가 880줄로 800줄 경고선 초과(receipt 스테이지 블록 `:430-827` 분리 후보) · fallback id 가 아직 결정적이지 않다 · `ledger-vocab-allowlist.test.js:11` 산문 "36 names" 가 낡았다. V5 §4 ④ 분모 착지(축 판정은 여전히 미판정).
- **resume-contract-dr02** (`2df68903`, p2): `lib/checkpoint/resume-controller.js` + `lib/supervisor/lane-reconcile.js` 신설(둘 다 report-only) + `commands/resume.md` 규약 + `tests/firewall/resume-contract-report-only.test.js` 포함 테스트 3 (6 files +1,875/−1). **apply 는 이 줄기 밖**이다 — `/resume --contract` 실행 스크립트가 없고, `lib/supervisor/index.js` 배럴에 미등록이며, Canary 단계 10(lease 회수·모델 전환·스냅샷)은 미구현. `run.json` 어휘가 `LANE_OPS_STATES` 밖이라 라이브 전 레인이 `reconcile:ops-state-unknown` 으로 읽힌다(split-ops 영역). revision 표기가 셋으로 갈린다: store 중첩 `intent.revision` · 체크포인트 스키마 최상위 `intent_revision` · 원장 `plan_revision`(공급 0) — schemas 정본 결정 필요. V5 SH-13 in-progress 유지(evidence 갱신).
- **guard-command-position** (`10cb53ff`, p3): `lib/core/command-segments.js` 신설로 L1/L2 가드가 **명령 위치**를 앵커해 `echo "rm -rf /"` 같은 언급형을 실행형과 가른다 + `lib/core/guard-registry.js`·`lib/autopilot/safety.js`·`lib/security/human-gates.js` 배선, 테스트 7 (11 files +1,925/−57). 실측: 언급 8래퍼 0/39 발화(`echoBare` 4/39 는 정당) · 실행형 양성 손실 0(2,872건에서 L1 490→476, 손실 14 전부 언급형) · PARITY 38→50(agreed 44 / owner-decision 6) · 전체 PreToolUse 최악 40,962B 에서 35.0ms. 리더 실측 재현 사례: 부모 세션 Bash heredoc 본문의 루트 rm 문자열에 L1 "rm -rf with path" 가 발화했다. **못 보는 것**: L2 `sql-delete-no-where` 가 `DELETE FROM t WHERE id=1` 을 danger 로 오판(몸통 클래스가 공백을 포함해 `t WHERE id` 까지 삼킨 뒤 lookahead 성립) — 핀만, 수리는 소유 밖 · L2 `rm-rf-root` 가 루트 타깃 직후의 따옴표·괄호를 못 잡는다(L1 block / L2 safe, owner-decision 핀) · `normalizeCommand` 의 `$(…)`·`${…}` 2차식은 `{0,192}` 로 줄기 안에서 수리했으나 **정적 스캔 3번째 카탈로그가 이 두 정규식을 보지 못한다**(구조적 사각) · `leader-addendum.md` B 의 수치가 실제보다 2.2~3.3배 과대(122,880B curl 1,839.8 / git push 684.8ms) → addendum 정본 갱신 필요 · 120KB 개행 `<50ms` 핀은 병렬 104ms 라 구조 단언으로 대체(gotcha 85). V5 §3 부채 done.
- **verify-gate-wiring** (`de8d238a`, p3): Stop 훅 `scripts/hooks/dev-verify-gate.js` 가 판정을 원장 분모행으로 쓰고(전 층 UNMEASURED), CLI `scripts/ledger/record-verify.mjs` 가 분자행을 쓴다 + 테스트 3 (6 files +1,361, 삭제 0). 리더 결정 L-3(분모행·분자행 둘 다) 이행. 순서는 **stdout → 원장**으로 번복했다 — 근거는 `_dispatcher-utils.js#spawnHook` 이 타임아웃 시 수집된 stdout 으로 resolve 한다는 것이고, 선례 `handleStop` 과의 차이는 훅 주석에 남겼다. CLI exit 규약은 브리프 문구가 아니라 SH-27 선례(사용 오류 exit 2 · 기록 실패 exit 0 `recorded:false`). **못 보는 것**: 설치본 미갱신이라 **라이브 분모 0** · `record-verify.mjs` 호출률을 잴 수단이 0 · `saveFingerprint` 실패 시 이중계수 승계 · Windows 파이프 flush 미확인 · 지연 N=20 이 비통제(972 → 1,084 → 1,665ms median, 다른 창 동시 가동) 이고 `existingKeys` 는 여전히 선형 판독(200k 행 572ms) → tail-read 후속. **(정정) SubagentStop 은 이 게이트를 타지 않는다** — dispatch-table 의 Stop 슬롯만이다. V5 OB-07 in-progress 유지(호출자 ≥1 은 충족, 층별 UNMEASURED 분모는 여전히 0).
- **review-md-writer** (`15af93e2`, p3): `lib/review/review-artifact.js` 신설(review.md 직렬화기) + SubagentStop 경로 `scripts/hooks/_review-stop-record.js` 배선 + 테스트 2 (4 files +1,948/−5). `review.completed` → review.md 왕복을 테스트로 실증했다. **라이브는 0 이 정답**이다 — 4.61.0 이 `runtime.artifactLifecycle.enabled=false` 로 출하됐다(리더 결정 ①). **못 보는 것**: `plan.revised`·`mission.completed` 는 **emitter 가 0** 이라 plan.md·outcome.md 생성 경로가 없다(`ledger.js:484` 로 미션이 영구히 열린 상태) · review revision 승계는 설계 §7.2 결정 대기 · apply 의 "never clobbers" 는 프로세스 간 TOCTOU 가 있다(`file.js:139-149`, 발생률 미측정) · `BASED_ON_MEMBERS_BY_KIND` 가 미export 복제 상수 · UTC 자정 straddle fallback id · 테스트 935줄(>800). V5 SH-02 in-progress 유지(review.md 경로만 착지).
- 운영 사건 ①(팀원 세션 한도): 2026-09-14 01:35 KST 에 **8창 전부의 opus 팀원이 "session limit · resets 2:30am" 으로 동시 사망**했다. 산출물 유실은 0 — 각 창이 wip 트레일러로 보호했다(gotcha 81). 리더가 상태를 확인한 것은 09:05(창 idle 약 7시간 30분)이고 8줄기 done 보고는 09:16~09:59 에 몰렸다. 원인은 계정 단위 한도로 **추론**한다(호스트 계측 없음).
- 운영 사건 ②(plan base 이동): plan 의 base `7dd14049` 가 worktree 분기점 `a81ee154` 와 달랐다(docs 2커밋 차이). `land.mjs:115` 가 `plan.base..branch` 를 diff 하므로 그대로 두면 모든 줄기의 소유권 검사에 무관한 6파일이 누출될 예정이었다. **8개 worktree HEAD 가 전부 `a81ee154` 이고 줄기 커밋 0** 임을 실측한 뒤 plan·run 의 base 를 `a81ee154` 로 옮기고 `planBaseOriginal` 을 보존했다(잡아낸 것은 session-end 창). 또 `dispatch.mjs` 는 `brief.md` 만 복사하므로 `leader-addendum.md` 는 리더가 수동 복사했다(guard 줄기).
- V5-BACKLOG §1 진행률 39% → **40%**(done 39 → 40 = OB-21 추가, Observe 18 → 19). done 으로 올리지 않은 것 4건 — OB-07(호출자는 생겼으나 라이브 분모 0) · SH-02(review.md 경로만, plan/outcome emitter 0) · SH-13(report-only 만, apply·`/resume --contract` 부재) · SH-16(Wave 8 부터 유지). §3 부채 트랙은 2건 done(guard-command-position · split-ops-dispatch-land).

## [4.61.0] — 2026-09-14

### 행동 변화 고지

① 4.61.0 은 PreToolUse `Write|Edit` 훅 `intent-observe-pre` 를 추가해 세션의 첫 쓰기 시점에 미션을 확정(`mission.created`)하고 `.artibot/missions/<id>/intent.md` 를 쓸 수 있는 writer 를 탑재한다. 다만 Observe 종료 판정이 끝나지 않아 `runtime.artifactLifecycle.enabled` 는 `false` 로 출하하며, 이 값이 false 인 동안 원장·상태 기록은 그대로 남고 파일은 어떤 프로젝트에도 생성되지 않는다. Shadow 진입 릴리스에서 이 값을 `true` 로 바꾸면 플러그인이 설치된 모든 프로젝트에서 첫 Write/Edit 마다 추적 경로 `.artibot/missions/` 아래 intent.md 가 생기므로, 그때 별도 고지한다. 프로젝트 단위 끄기는 아직 없다(loadConfig 단일 경로). 이 값이 false 인 동안에는 문서가 생기지 않아 fast latch 가 걸리지 않으므로 훅이 매 Write/Edit 마다 원장을 한 번씩 통독한다(원장 크기에 비례, 상한 없음). 이 리포 실측(2026-09-14 00:12, `.git/artibot/ledger.jsonl` 305,435B/268행) read+JSON.parse 1.4~2.2ms ×5회로 PreToolUse 5초 예산 대비 무시 가능하나, 라이브 훅 왕복 지연은 미측정이다.

② PostCompact 훅이 Context Receipt(`context.compiled`)를 조립하지만 컴팩션 시점에는 스키마 10키 중 6키만 실측 가능해(mission_id·based_on·transforms 5·cache 3 결손) 원장에는 발행하지 않고 결손 목록만 기록한다. 원장 어휘 allowlist 의 `context.compiled.sources` 는 `worker` 만 허용하며, 훅 경로가 영수증을 완성할 수 있을 때까지 이 제한을 유지한다. 라이브 원장의 `context.compiled` 행 수가 0 인 것은 결함이 아니라 이 규칙의 결과다.

③ RouteBench 기준선 B0~B6 와 오프라인 러너가 추가되었고 예시 시나리오 2건은 `fixture-pending` 이라 이번 릴리스에서 실제 채점되는 행은 없다. B4(v5 적응 라우터)는 현재 classifier 에 agentType 을 넘기지 않아 모든 시나리오가 기본 클래스(implement)로 랭크되며, 이는 라이브 `route.selected` 가 쓰는 경로(`route-observe-pre.js` `input:{agentType}`)와 다르다. 다음 웨이브에서 B4 정의를 `input:{agentType}` 공급으로 바꿀 예정이며 그때 `baselines_sha256` 이 바뀌어 이전 로컬 리포트와 비교되지 않는다.

### Wave 8 배치 착지 (split-68e984w8, base a40b8448 → p1 e9e24e2e → p2 34b6dbf6 → p3 b906264a → p4 9300568e → p5 ee86a639, 8/8줄기, 74 files +14,989/−277)
...
- **context-receipt-cx02** (`00c5fad1` · `7e9ea9c7` · `576a99d5` · `8cea8615` · `fc21e99e`, p3): `lib/context/context-pressure.js`(순수 점수, 호스트 `context_window.max_tokens` 없으면 null — 카탈로그 폴백 제거) · `context-receipt.js`(조립기, dotted missing) · `rehydration.js#reportContextReceipt`(writer 주입 포트) + PostCompact 배선 (10 files +2,029/−81). 실측 핀: hook 소스 `context.compiled` 거부 / worker 수락 2×2; PostCompact 시점 필수 10키 중 6(라이브 5)만 채움 → **라이브 발행 0 은 구조적**. V5 SH-16 in-progress 유지.
- **intent-md-generator** (`b4b900fd` · `0f4b9a1e` · `87617ad9` · `a2434da0` · `50ad3c37` · `17f3e87c` · `7d75031a`, p4): PreToolUse `Write|Edit` 훅 `scripts/hooks/intent-observe-pre.js`(stage-② S1 확정 → `mission.created` + `missions/<id>/intent.md`, stdout 바이트 불변 3케이스, UTC 자정 세션 id 결함 수리, lib 지연 import) + `lib/runtime/artifact-lifecycle.js#apply` 게이트 3 writer + `tasks.js` title 운반 + hooks.json 슬롯 1 (10 files +2,252/−42 + 핀 갱신 2). C9 베이스라인 created/(created+deferred) = 20/243 = 8.23%(stage-② 배포 전). **4.61.0 은 `runtime.artifactLifecycle.enabled` 를 `false` 로 출하(결정 ①) — writer 는 탑재되나 파일 생성 0, 원장·상태 기록은 그대로. ON 전환 = Shadow 진입 릴리스**. V5 SH-01 done.
- **routebench-baseline** (`51872c8d` · `d4db5fb5` · `560d1bce`, p4): `scripts/bench/routebench.mjs` 오프라인 채점 러너 + `baselines.{json,schema.json}` B0~B6(B2 = `resolveModel` 호출, B3/B4 `inferred`, B6 `unimplemented`) + `scenarios.schema.json` 선택 `agentType` + 테스트 2 (6 files +2,221). 검수 R1 BLOCKER: 러너가 `loadConfig` 를 안 불러 fable 게이트 꺼진 채 채점(테스트도 같은 방식 — 수렴≠검증) → `validate-model-policy.js` 패턴으로 수리, 봉투 `policy_source`·`baselines_sha256`; pairKey NUL 바이트 `no-control-bytes` 게이트로 수리. V5 SH-25 done.
- **verdict-verify-writer** (`67cce028` · `57236e1b` · `d9b6ff7d` · `c55197e2` · `e00985f5` · `d9d21a9b` · `1c55b440` · `7298bc34`, p4): `lib/review/{verdict-writer,claim-audit-writer}.js` · `lib/verification/verify-writer.js`(pure + ports) + SubagentStop 배선 `scripts/hooks/_review-stop-record.js`(핸들러 800줄 게이트로 순수 분리, `last_assistant_message` 실림 실측, stdout 불변 4케이스, 8MB 꼬리 회귀 2) + `spawn-ledger.js#OPTIONAL_FIELDS` `review_ledger` 1행 (11 files +3,443/−10). 라이브 원장 233+349행 모두 `review.*`·`verify.completed` 0 → 파싱률·층별 UNMEASURED **분모 0 미측정**(호출부 미배포). V5 OB-06 done, OB-07 in-progress(런타임 호출자 0).
- **human-resolved-writer** (`4ac149c4` · `e5fcb88e` · `c20ca01f`, p5): `lib/runtime/human-asked-record.js#recordHumanResolved`(kind 3, 스킵 5, decision 3,072B 사전검사) + CLI `scripts/ledger/record-human-resolved.mjs`(`question_id` 를 asked 와 같은 경로로 재계산, 실제 pre-write 훅 spawn 대조) + `.artibot/project.md` §Human Approval Boundaries 규약 7줄 (5 files +1,475/−29). (a) PostToolUse 는 페이로드에 subject 가 없어 부적합 → (b) CLI; 호출 주체 아직 0. V5 SH-27 done.
- README 카운트 동기화(`c23eb5c4`, p4): 훅 등록 27→28, 훅 스크립트 70→72 — 줄기가 아니라 리더가 병합 미리보기에서 1회(3줄기 동시 변경 충돌 방지).
- p4 1차 `ca0f0da7` not-green: `tests/hooks-schema-shape.test.js` 6b `PRETOOLUSE_INTENT` 누락 + 지문 불일치(intent 의 hooks.json 슬롯) → 줄기 `7d75031a`(`WRITE_TOOLS` 4종, 지문 `4e0039e6…`) → 2차 초록. 리더 plan 결함: 설정 파일 핀 테스트가 allowlist·표적 스위트 어디에도 없었다(land 의 merge-tree·표적 스위트는 파일 간 핀을 못 본다).
- V5-BACKLOG §1 진행률 33% → **39%**(done 35→39: SH-01·SH-25·SH-27·OB-06; Shadow 3→6). 창 운영: 2026-09-13 21:4x 창 8개 전부 사망 상태에서 5창 재개설(`claude --worktree` 기존 worktree 재사용 실증) → dispatch 재발행 → 5줄기 착지까지 약 65분(21:57 포인터 → 22:57 p5). 측정 고지: 사람 대기 0.8%(humanWaitMs 1,405,752 / run 169,993,820ms, 미쌍 1 = `wait-limbs`), `humanWaitReevalPct`=50 대비 미만.
...
### /split Wave 5 (split-68e984, 배치 랜딩 dcd4ce5a · 2026-09-11)

#### Fixed
- **`/doctor` Check 8 이 프로젝트명을 `'artibot'` 으로 접음** (23ec79be·d1ded07f, `lib/project-state/doctor-checks.js#checkLedgerStateParity` · `commands/doctor.md`): 문자열 projection(= `state.yaml` 원문)을 받으면 객체 분기를 못 타 이름이 `'artibot'` 으로 접혔고, 그 값은 `lib/project-state/journal.js` 빈 스냅샷 기본값일 뿐 정본이 아니다. 정본은 `path.basename(projectRoot)`(`lib/project-state/state-manager.js#createProjectStateStore` 와 같은 규칙 — config 도 package.json 도 아니다). 우선순위를 `project 인자 > 문자열 `project:` 행 파싱 > 객체 `.project`` 로 두고, **파싱 실패 시 기본값으로 접지 않는다** — `project-name-unresolved` finding + `unmeasured`(fail-open 금지). `commands/doctor.md` Check 8 Step 0 에 이름 출처를, 호출 예에 `project` 인자를 명시. journal 은 이름을 싣지 않으므로(부모 실물 21행 중 0행) 문서만 고치면 다음 호출자가 같은 함정에 빠진다. **못 보는 것**: projection 에서 읽은 이름으로 그 projection 을 대조하므로 `project` 행 **자체의** drift 는 잡지 못한다 — journal 에 이름이 없어 잃는 검증력은 0 이다. 4.57.0 Known 의 후속이다.
- **L1 정규화가 줄바꿈을 접어 안전한 2줄 스크립트를 차단** (3ca95219, `lib/core/guard-registry.js#normalizeCommand`): 마지막 접기 `/\s+/g` → `/[^\S\n]+/g`(줄 안 공백만, 외톨이 CR 포함), `\r?\n` 은 `\n` 하나로. `checkDangerousCommand` 가 원문과 정규화 변형 둘 다 대조하므로 종전에는 줄바꿈이 사라진 변형이 `git branch -d old echo -f done` 이 되어 L1 `git branch -D (force delete)` 에 걸렸다(CRLF 동일). 이제 두 층이 같은 줄바꿈 의미론을 쓴다. `$`·`\s*$` 앵커를 쓰는 기존 규칙들이 다중 줄 입력에서 완화되지 않는지를 규칙 전수로 대조했다(회귀 0건).

#### Added
- **L1 `dd` 디바이스 쓰기 규칙 + fork bomb 규칙 재작성** (3ca95219, `lib/core/blocked-patterns.js`): 신규 `dd write to block device`(`\bdd\b[^\n]{0,192}\sof=\/dev\/`) 를 추가하고 기존 `dd raw disk write`(`dd\s+if=`)는 **유지** — 대체가 아니라 추가다(`tests/hooks/pre-bash.test.js` 가 `dd if=/dev/zero of=/dev/sda` 의 라벨을 핀하고, 호출자는 첫 매치를 취한다). 창 폭 192 는 L2 쌍둥이 규칙과 같은 값이고 사유도 같다(ReDoS). fork bomb 규칙은 종전 `/:(){ :\|:& };:/i` 가 두 가지로 틀렸다 — `()` 가 빈 캡처 그룹이라 정규식이 실제로 요구한 문자열은 셸이 실행할 수 없는 `:{ :|:& };:` 였고(진짜 fork bomb 3형태 전부 APPROVED, 2026-09-11 executeChain 실측), 카테고리도 `network` 였다(로컬 프로세스 슬롯 고갈이므로 `system`). 괄호를 이스케이프 + 선택으로 바꿔 **괄호 없는 종전 형태를 포함하는 순증**으로 재작성. 1차안 `:\s*(?:\(\s*\))?\s*\{` 는 선택 그룹이 두 `\s*` 사이에 놓여 **2차식**이었고(검수가 잡았다: 40KB 공백 1,255ms · 120KB 17,199ms — 3배 입력에 13.7배 시간) 당시 콜론 밀집 입력만 쓰던 테스트는 그 위에서 초록이었다. 확정형 측정치는 40KB 공백·개행 각 0.1ms, 120KB 0.5/0.4ms. 불변식은 "선택 그룹만으로 분리된 `\s*` 쌍을 만들지 않는다" 이며 주석에 고정했다.

#### Changed
- **릴리스 산문 동기화를 테스트 수 집계 뒤로** (dc7694a1·8310580f·c298a7f0, `.github/workflows/release.yml` `sync-readmes`): `Sync README count prose` 가 `Count test cases` · `Sync marketplace.json metadata` 보다 **먼저** 돌아 `marketplace.json#qualityMetrics.tests` 만 새 값이 되고 같은 수치를 인용하는 산문 4곳은 옛 값으로 남았다. 그 브랜치 CI 의 `validate-readme-claims.js --full` 이 드리프트로 실패하고 배지 ff 가 이슈로 에스컬레이션된다 — v4.58.0(#117) · v4.59.0(#118) 두 릴리스 연속 재발, 둘 다 손으로 수습해 랜딩했다. 이제 산문 동기화가 marketplace 집계 **뒤에** 온다. 이 순서는 `tests/firewall/` 의 release.yml 슬라이서 게이트가 핀한다. **이 줄기는 라이브 실증을 하지 못한다**(태그 푸시 = 릴리스) — 실증 항목은 v4.60.0 릴리스 런의 `sync-readmes` 로그와 `ci/sync-badges-v4.60.0` CI 색이다.
- **SYNC_PATHS 를 9경로 단일 목록으로** (8310580f·c298a7f0, `.github/workflows/release.yml`): 동기화·`git add`·변경 감지가 각자 파일 집합을 들고 있던 것을 잡 env 의 한 목록으로 모으고, 변경 감지 주석을 그 목록 기준으로 갱신. 뒤에 경로를 더해도 감지 루프가 자동으로 따라가도록 catch-all 을 의도적으로 남겼다.

### /split Wave 6 (split-68e984w6, 배치 랜딩 663411a2 · 2026-09-11)

#### Changed
- **`split.maxWindows` 기본값 4 → 8 lockstep** (395a0185, `artibot.config.json#/split/maxWindows` · `tests/firewall/split-config-firewall.test.js` · `commands/split.md` · `skills/split/SKILL.md` · `tests/firewall/v5-config-firewall.test.js`): 오너 2026-09-11 처리량 우선 지시. 하드캡 `lib/.../fast-profile.js#HARD_MAX_WORKTREES` = 12 는 불변이고 새 키 0 이다. 값·게이트·커맨드 문서·스킬 문서를 한 커밋에 맞춰 드리프트를 막았다.

#### Fixed
- **L2 `dd`·`curl`·`wget`·`git push` 규칙의 2차식 스캔** (8f473a9e, `lib/autopilot/safety.js`): 네 계열 모두 "<명령어> <임의> <토큰>" 형태에 `[^\n]*` 를 써서 명령어가 나올 때마다 줄 나머지를 재스캔했다 — 40,962B 비매칭 입력 3회 실측 `dd` 852.1/863.7/759.9ms · `curl` 461.7/500.5/444.3ms · `wget` 483.3/498.2/504.8ms · `git push` 3규칙 합 600.3/630.3/645.8ms, 120KB 에서 `dd` 6,306.1ms · `git push` 5,236.0ms. `{0,192}` 창으로 바꾼 뒤 각각 15.4/14.2/14.4 · 7.6/7.4/7.6 · 7.9/7.5/8.4 · 15.7/16.5/13.8ms, 120KB 35.4 / 21.5 / 23.5 / 41.5ms(node v24.15.0, 2026-09-11 06:36~06:47 UTC). 기준선 50ms 는 `bash-risk-guard.js` 가 5초 PreToolUse 훅 안에서 도는 데서 온다. **이 창이 못 보는 것**(초록 스위트를 커버리지로 읽지 마라): 명령어와 토큰 사이가 192자를 넘으면 규칙을 통째로 회피한다 — 긴 피연산자 목록, 거대한 이미지 경로, 플래그 더미 뒤에 묻힌 URL, 줄 한참 뒤의 `| sh`, 강제 푸시 플래그 앞의 많은 refspec 또는 긴 원격 URL 이 전부 그 사각지대에 있고 전부 safe 로 판정된다. L2 에서 이를 잡는 다른 규칙은 없다. 창을 넓히는 것은 실제 선택지이되 120KB 재측정이 선행 조건이다. `git push` 3규칙은 하나를 더 내준다 — 런이 셸 구분자에서 멈추므로 **따옴표 안의** 구분자도 런을 끝낸다: `git push "a;b" --force` 는 safe 다(2026-09-11 실측, 구분자 정지 도입 전에는 danger). L2 는 원문을 읽고 셸 파싱을 하지 않아 인용부호를 구별할 수 없다. 구분자 정지가 없애는 오탐 3건과 맞바꾼 것이 2026-09-11 결정이며 양쪽 다 실재한다.
- **`git checkout -- .` · `git restore -- .` 가 L1 을 통과** (8f473a9e, `lib/core/blocked-patterns.js`): L1 두 규칙이 `--` 구분자 형태를 놓쳐 L2 danger / L1 approve 였다 — 방향 규칙(L1 block ⇒ L2 ≥ caution)의 반대 방향 구멍. L2 와 같은 형태로 맞췄다. 경로 지정 형태(`git restore -- file.js` 등)는 계속 통과한다.
- **`--force-with-lease` 면제가 무검사 `--force` 까지 덮음** (8f473a9e, `lib/core/guard-registry.js#checkDangerousCommand` · `lib/core/blocked-patterns.js`): 면제가 **명령 전체**를 대상으로 검사돼 `git push --force-with-lease --force` · `git push -f --force-with-lease` 가 L1 approve / L2 danger 였다. L1 패턴을 L2 와 동형(`--force` 뒤 lease/if-includes 부정 선읽기)으로 바꿨다 — `git-branch-delete` 를 두 층 바이트 동일로 유지한 4.59.0 선례를 따른다. 검사형 푸시(`--force-with-lease`·`--force-if-includes`)의 L1 pass / L2 caution 은 무변경.
- **낡은 가드 주석 2건** (8f473a9e, `scripts/hooks/permission-auto-approve.js` 헤더 · `lib/autopilot/safety.js` 비대칭 주석): 헤더의 "lease 는 blocked-patterns 면제인데 classifyRisk 는 danger, raw-disk dd 는 가드가 막는데 classifyRisk 는 safe" 는 4.59.0 시점에 이미 거짓이었고(lease = caution, dd = danger) 이번 변경으로 한 번 더 바뀌었다 — 착지 상태로 재서술. `normalizeCommand` 를 "out of scope, owner-decision 행으로 핀" 이라 적던 문단은 Wave 5 에서 해소됐으므로 사실에 맞게 갱신.
- **프론트매터 중복 키 2건 + 재발 게이트** (62d4d608, `skills/git-unified/SKILL.md` · `skills/lang-reference/SKILL.md` · 신규 `tests/ci/frontmatter-duplicate-keys.test.js`): 두 SKILL.md 가 `platforms:` 를 두 번 선언했다(같은 값). 검증기 어디에도 중복 키 검사가 없었으므로 skills·agents·commands 전 프론트매터를 훑는 게이트를 신설하고, 픽스처에 중복을 심어 red 를 확인하는 **스캐너 자기검증**을 함께 넣었다.
- **`extractFrontmatter` 가 블록 스칼라를 값 `"|"` 로 저장** (62d4d608, `scripts/ci/ci-utils.js#extractFrontmatter`): `description: |` 형태에서 본문이 아니라 파이프 문자가 값이 됐다. `validate-skills.js` 는 자체 우회로 막고 있었으나 나머지 소비자 3종(`validate-agents.js`·`validate-commands.js`·`validate-model-policy.js`)은 존재 여부만 봐서 빈 블록 스칼라가 통과할 수 있었다(현 코퍼스 실해 0). 파서가 들여쓰기 본문을 접어 값으로 저장하고 빈 본문은 `''`(falsy) 로 돌린다. 인라인 값과 CRLF 동작은 무변경.
- **`lang-reference` description 이 호스트 캡 초과** (62d4d608, `skills/lang-reference/SKILL.md`): 호스트 description 캡을 넘던 유일한 스킬을 캡 아래로 축약했다 — 스킬 활성화는 이 문자열 매칭으로 일어나므로 잘림은 활성화 실패로 이어진다. R1 이 요구하는 트리거 어휘(≥3, 한/영 병기)는 보존했다. 린터 warn 은 유지가 결정이므로 `polish` 는 무접촉이다.
- **`atomicWrite` 가 Windows EPERM 에서 즉시 실패** (c571b49d, `lib/handoff/handoff-store.js`): `rename` 이 다른 프로세스의 열린 핸들과 경합하면 tmp 를 지우고 곧장 throw 했다(주석은 원인을 알고 있었다). 짧은 백오프 재시도를 넣되 **`EPERM` 만** 재시도한다 — `ENOENT`·`EACCES`·`EBUSY`·`ENOSPC`·`EXDEV` 등 다른 errno 는 종전대로 즉시 throw 이고, 재시도를 모두 소진하면 tmp 잔존 0 으로 throw 한다. `rename` 을 주입 가능하게 열어 재시도 경로와 영구 실패 경로를 테스트가 단언한다. 단일 프로세스 반복 실행에서는 EPERM 이 한 번도 관측되지 않았다 — 재현은 별개 프로세스 동시 `/save` 에서만.

#### Added
- **공유 원장 위치 — ADR-011** (f4882946·921dcf3c·a1be029c, 신규 `lib/project-state/store-location.js#resolveStoreLocation` · `lib/runtime/event-writer.js#ledgerFilePath` · `.artibot/adr/ADR-011-worktree-ledger-store.md`): 원장 위치 규칙을 StateStore 와 한 모듈로 합쳤다. 명시 `ledgerPath` 는 현행 유지, git 이면 `<git-common-dir>/artibot/ledger.jsonl`, 비-git 이면 종전 `<projectRoot>/.artibot/runtime/ledger.jsonl` 과 **바이트 동일**. 새 모듈은 fs import 0 의 순수 path 모듈이고 `state-manager.js` 가 같은 이름으로 re-export 하므로 기존 import 는 무수정이다. 의존 방향은 runtime → project-state 하향. 이번 줄기는 `ledger.jsonl` **하나만** 옮긴다 — `spawns.ndjson` 은 다음 웨이브이고, 그 사이의 비대칭(원장은 전 창 합산 · spawns 는 트리별)을 `/doctor` Check 10 의 "cannot see" 절에 문장으로 남겼다. 게이트: 합성 linked worktree 에서 메인·워크트리 두 루트의 `result.path` 가 같은 파일을 가리키는지, 비-git 폴백이 바이트 동일인지, 실 커밋 뒤 양 루트의 Check 8 에 subset 위반이 없는지, 실제 `git worktree add` 경로 1건. 동시 append 는 기존 3프로세스에 8프로세스 케이스를 더했다. **이 게이트들이 못 보는 것**은 각 헤더에 그대로 남겼다(프로세스 수 상한 초과 · 4KB 경계 근접 · 네트워크 파일시스템 · 라이브 페이로드). 구파일은 `ledger.jsonl.pre-adr011` 로 개명 보존하며 **실 이관과 삭제는 이 배치 밖**이다(드라이런 증거만; 삭제는 릴리스 태그 뒤 체크리스트 항목).
- **벤치 스크립트 + `benchmark.include` 명시** (94575a53·4c997b6c, `vitest.config.js` · `package.json`): `.bench.js` 가 기본 글롭에 의존하던 것을 프로젝트별 명시 include 로 고정하고 `bench`·`bench:hooks` 스크립트를 추가했다. **`ci` 체인에는 넣지 않는다** — 4분이 걸리고 다른 세션이 스토어에 쓰는 중이면 tree guard 가 exit 2 다.

#### Docs
- **주석·문서 정합 6건** (395a0185): ① 디스패처 5개 테스트 헤더가 `scripts/hooks/_dispatcher-utils.js` 의 spawn 위치를 **줄번호로** 인용해 이미 어긋나 있었다 → 심볼 `#spawnHook` 인용으로 바꿔 다시 썩지 않게 했다. ② `skills/git-unified/references/safe.md` 가 "전체 되돌리기" 예시로 PreToolUse 가 L1 block · L2 danger 로 막는 명령을 제시하고 있었다 → 경로 지정 형태로 바꾸고 "전체 되돌리기는 가드가 차단한다(사람이 직접)" 고지 1줄. ③ `release.yml` 의 "committing the four files" → 실제 정본인 `SYNC_PATHS`(9경로) 표기로. ④ 같은 워크플로의 agents 카운트가 `INDEX.md` 를 세어 31 을 찍었다(로스터 30) → `! -name INDEX.md`. 이 출력은 진단용이고 소비처는 0 이다. ⑤·⑥ `skills/split/references/operations.md` · `commands/split.md` 에 **창 닫기 순서**를 명시: `--teardown`(junction 제거) → 죽은 pid 확인 → `git worktree unlock` → `git worktree remove` → `git worktree prune`. teardown 을 remove 뒤로 미루면 junction 이 남는다 — 2026-09-11 정리에서 실제로 13개가 남았다.
- **`badge-stall` 게이트 1,033줄 분할** (9bf974b9, `tests/firewall/badge-stall-*`): 헬퍼 모듈(비테스트 `.js`) + 라이프사이클 · 동기화 순서 · 랜딩 lockstep 3개 테스트 파일 + 헬퍼 자기검증 테스트로 나눴다. **순수 이동** — `it` 문자열과 단언 바이트는 불변이고, 각 파일에 원본의 "이 블록이 못 보는 것" 절을 남겼다.
- **민감어 파일명 때문에 편집 불가하던 테스트 개명** (201bbea9, `tests/firewall/release-landing-credentials.test.js` → `release-landing-push-identity.test.js`): 가드의 민감 파일 패턴이 basename 의 `credentials` 에 걸려 Write/Edit 도구가 이 파일을 **내용과 무관하게** 거부했다(2026-09-11 실발생). 내용은 release.yml 정적 스캐너 테스트다. 가드는 **무수정** — 게이트를 통과시키려 게이트를 깎지 않는다. 새 이름이 민감 패턴 전 항목에 불일치함을 먼저 증명하고 개명했고, 단언은 무수정이다.

#### 리더 통합 시 동승 (docs)
- `lib/observability/decision-events.js` JSDoc 예시의 `policy:split.maxWindows=4(observe-only)` → `=8`(주석 1줄, 코드 토큰 0). 소유 줄기 0, 핀 테스트 0.
- `tests/firewall/split-dispatch-idempotency.test.js` 헤더 "이 픽스처가 못 보는 것" 절의 `실용 상한 4창` → `상한 8창`(주석 1줄, 단언 무수정).
- `skills/adr-format/SKILL.md` · `skills/persona-distill/SKILL.md` 의 `source_hash` 1줄씩 갱신(114개 중 2개가 stale). `scripts/inject-source-hash.js` 는 **돌리지 마라** — 114 파일을 전부 LF 로 다시 쓴다. Edit 로 해당 줄만.
...
#### Changed
- **`git-force-push` 분리 + `git-force-push-lease` 신설** (`lib/autopilot/safety.js`): `--force(?!-with-lease|-if-includes)` 부정 선읽기로 무검사 강제 푸시만 danger 로 남기고, 검사형(lease/if-includes)은 새 id `git-force-push-lease` 의 caution 으로 뺐다. L1(`lib/core/blocked-patterns.js` 두 `git push` 규칙의 `safeOverrides`)이 같은 두 플래그를 면제하고 있어 두 층이 같은 명령에 정반대 판정을 내던 것을 해소. `git push -f` 는 `git-force-push-short` 로 danger 유지.
- **`dd-device-write` 신설** (`lib/autopilot/safety.js`): `dd` 명령의 `of=` 가 `/dev/` 를 가리키면 danger. L1 은 `dd\s+if=` 로 모든 dd 를 차단하는데 L2 가 safe 였다 — "L1 block ⇒ L2 최소 caution" 방향 규칙 위반이었다. 파일 대 파일 dd(`dd if=a.img of=b.img`)는 이번 결정 범위 밖이라 L1 block / L2 safe 불일치로 남는다.

#### Fixed
- **`sql-truncate` 단어 스침 오탐** (`lib/autopilot/safety.js`): `/\bTRUNCATE\b/i` → `TRUNCATE {TABLE|ONLY|DATABASE} <식별자>` 또는 `TRUNCATE <식별자 목록>` + 종결자(`;`·`CASCADE`·`RESTRICT`·`RESTART/CONTINUE IDENTITY`). 양성 대조 5종(`TRUNCATE users;`·`TRUNCATE TABLE users`·`TRUNCATE ONLY users CASCADE`·`psql -c "TRUNCATE users RESTART IDENTITY"`·`TRUNCATE DATABASE foo`)과 음성 대조 8종(grep 인자 2종·`git log --grep=truncate`·`echo truncate`·`fs.truncateSync`·coreutils `truncate -s 0`·커밋 메시지·`rg`)을 테스트로 고정. **못 보는 것**: 키워드도 종결자도 없는 `psql -c "TRUNCATE users"` 는 잡지 않는다 — 닫는 따옴표를 종결자로 넣으면 산문이 다시 걸려서 의도적으로 남긴 구멍이다. **잘못 보는 것**: `echo "truncate cache;"` 처럼 한 단어 뒤에 세미콜론이 오는 산문은 danger 로 본다 — 드물고 실패 방향이 차단 쪽이라 수용한다. ReDoS 타이밍 단언 추가.
- **`git branch -d` 가 두 층 모두에서 강제 삭제로 취급됨** (`lib/autopilot/safety.js` `git-branch-delete` · `lib/core/blocked-patterns.js` `git branch -D (force delete)`): 두 규칙 다 `/i` 를 달고 있어 소문자 `-d`(병합된 브랜치만 지우는 안전한 형태)까지 잡았고, 실제로 PreToolUse 에서 작업이 막혔다. 양쪽에서 `/i` 를 제거했다. L1 의 종전 형태는 `-D` 가 `branch` 바로 뒤에 와야 해서 `-q -D`·`-Dv`·`--delete --force`·`-fd` 를 전부 놓치고 있었다 — 두 규칙을 **바이트 단위로 동일한** 형태로 맞춰 세 가지 강제 삭제 형태만 잡는다: 대문자 D 를 포함한 단문자 묶음(`-D`·`-qD`·`-Dv`·`-Df`), 삭제 플래그와 강제 플래그의 동시 등장(`-fd`·`-df`·`-f -d`), `--delete` + `--force`. **"동시 등장"은 문자 그대로다** — 옵션 런이 대시 없는 인수도 건너뛰므로 브랜치명 **뒤에** 붙은 플래그까지 잡는다(`git branch -d topic -f`·`git branch topic -D`; git 이 실제로 honour 하는 형태인데 종전에는 두 층 다 놓쳤다). 런은 셸 구분자(`;` `&` `|`)와 **줄바꿈**에서 끝나되, **백슬래시 줄 연속(`\` + 개행, CRLF 포함)은 한 명령으로 보아 런을 잇는다**. 줄바꿈 경계가 없으면 다음 줄의 `-f` 가 강제 플래그로 읽혀 `git branch -d old` + `rm -rf build` 2줄 스크립트가 오탐이 된다(검수 정규식 실측 오탐 2건, **L2 에서** 교정). **이 줄바꿈 경계는 L1 에는 닿지 않는다** — L1 은 `guard-registry.js#checkDangerousCommand` 가 `normalizeCommand` 변형(모든 `\s+` 를 공백 하나로 접음)도 대조하므로 패턴이 보기 전에 줄바꿈이 사라진다. 그래서 다중 줄 스크립트의 뒷줄 `-f` 는 **여전히 L1 에서 block, L2 에서는 safe** 다(2026-09-11 executeChain 실측, CRLF 동일). 근본 해결 후보인 `normalizeCommand` 의 `/\s+/g` → `/[^\S\n]+/g` 는 `BLOCKED_PATTERNS` 38규칙 전부에 영향을 주므로 별도 줄기 + 오너 결정으로 남기고, 이 불일치는 패리티 매트릭스의 `owner-decision` 행이 판정 경로에서 핀한다. 대소문자는 셋이 서로 다르게 처리된다: `git` 은 무시(셸이 `GIT branch` 를 실행한다), `branch` 는 소문자 고정(git 이 `git BRANCH` 를 "is not a git command" 로 거부한다 — 2026-09-11 실측), `-D` 는 구분(이 한 글자가 규칙의 전부다). 토큰도 구분자도 두 갈래로 파싱될 수 없어(옵션 갈래는 대시+`\w` 요구, 인수 갈래는 선두 대시 금지, 줄 연속 갈래는 선두 백슬래시라 공백과 배타적) 스캔이 선형이다 — 120KB 적대 입력 L2 1~3ms · L1 8ms 미만 실측(검수자 머신, 구분자 확장 후), 양층에 40KB 타이밍 단언 2종(대시 없는 인수 런 · 줄 연속 런) 추가.
- **L1/L2 패리티 매트릭스 갱신** (`tests/core/guard-registry-safe-override-scope.test.js`): 21→27행. `owner-decision` 3행(lease push · bare TRUNCATE · dd)의 목표값과 사유를 2026-09-11 확정 내용으로 교체하고 `git branch -D topic`·`git branch -d topic`·`git branch --delete --force topic`·`TRUNCATE TABLE users`·grep 음성 대조 5행 추가. dd·`TRUNCATE TABLE`·git branch 3행은 `agreed`. lease 행과 bare TRUNCATE 행, 그리고 새 다중 줄 행(`git branch -d old` + 개행 + `echo -f done`, L1 block / L2 safe — 위 `normalizeCommand` 사유)은 헤더의 방향 규칙을 충족하지 못해 `owner-decision` 으로 남는다 — 규칙을 고쳐 맞추지 않았다.
- **합집합 증거 테스트의 예시 명령 교체** (`tests/hooks/permission-auto-approve.test.js`): ①·③ 이후 종전 예시 두 개가 각자 증명하던 방향을 잃었다. "safety.js 만 잡는" 예시는 `git push --force-with-lease` → `git push -f --force-with-lease`(L1 은 safeOverrides 로 여전히 면제, L2 는 `git-force-push-short` 로 danger), "blocked-patterns 만 잡는" 예시는 `dd if=/dev/zero of=/dev/sda` → `dd if=a.img of=b.img`(L1 block / L2 safe). 단언은 무수정 — 불변식이 아니라 예시가 낡은 것이었다.

## [4.58.0] — 2026-09-10

### split-b87130 — 3 wave · 10 줄기 (2026-09-10, 배치 랜딩 d410ebb0 → 27510566 → 91f55319)

**행동 변화 고지.** ① UserPromptSubmit 훅이 피어/에이전트 메시지 봉투(`<cross-session-message`·`<agent-message` 등)를 사람 프롬프트로 라우팅하지 않는다 — 팀원 보고가 도착할 때마다 붙던 `[artibot:route …]`·`[auto-team-suggested]` 주입이 사라진다. ② `install.sh`/`install.ps1` 이 네이티브 플러그인 캐시를 감지하면 agents/commands flat 복사를 건너뛴다(`--flat`/`-Flat` 로 강제). ③ `permissions.autoApprove` 매치여도 파괴적·판정불가 Bash 명령은 자동 승인되지 않고 기본 프롬프트로 돌아간다(기본 OFF 불변). ④ 세션 시작 버전 체크를 `ARTIBOT_UPDATE_CHECK=0` 또는 `updateCheck.enabled=false` 로 끌 수 있다. ⑤ ledger reader dedupe 키에 `ts` 가 들어가 Windows pid 재사용 이벤트가 더 이상 duplicate 로 탈락하지 않는다(`/doctor` Check 8-①/9 영구 WARN 원인).
...
- `commands/doctor.md` Check 8 호출 예 `project` 인자 누락 → basename≠artibot 워크트리 전부 `projection-drift` 위양성(후속) → **4.60.0 W5-a(23ec79be) 로 수리**. 같은 세션 2번째 substantive 프롬프트마다 `mission.created` 재발행(기존, 판독 무해). 재개 세션 usage 증분은 과소집계(reader 최신값 우선이 정답, 후속). 이벤트별 `sources` 위젠 게이트 부재.
