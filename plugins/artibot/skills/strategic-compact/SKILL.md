...
컨텍스트 오버플로우 감지 및 출력 자동 절삭 파이프라인.

**1. 오버플로우 감지**
```js
import { ContextRecovery } from './lib/runtime/context-recovery.js';

const overflow = ContextRecovery.detectOverflow({
  currentTokens: state.tokenUsage.total,
  maxTokens: state.tokenUsage.limit,
  threshold: 0.85, // 85% 이상이면 overflow 판정
});
// { isOverflow: true, usage: 0.91, zone: 'red' }
```

**2. 절삭 제안 생성**
```js
const suggestion = ContextRecovery.suggestTruncation({
  sections: state.context.sections,
  priority: ['activeTask', 'decisions', 'fileState'],
  dropCandidates: ['exploratorySearch', 'failedAttempts'],
});
// { keep: [...], truncate: [...], estimatedSavings: 12000 }
```

**3. 출력 절삭 실행 (파이프라인)**
```js
const truncated = ContextRecovery.truncateOutput(suggestion, {
  preserveHeadings: true,
  maxSummaryLength: 200,
});
// 절삭된 컨텍스트 — preserve 항목은 원본, truncate 항목은 요약본
```

**전체 파이프라인 (한 줄)**:
```js
const result = ContextRecovery.truncateOutput(
  ContextRecovery.suggestTruncation({ sections, priority, dropCandidates }),
  { preserveHeadings: true }
);
```

**활용 시점**:
- PreCompact 훅 이벤트 발생 시 자동 호출
- Orange/Red 존 진입 시 미들웨어에서 트리거
- `detectOverflow()` → `suggestTruncation()` → `truncateOutput()` 순서로 체이닝
