...
| `safe_ours` | theirs in ours (ours가 theirs 포함) | 자동 — ours 채택 |
| `safe_theirs` | ours in theirs (theirs가 ours 포함) | 자동 — theirs 채택 |
...
```
자동 해결 완료:
  login.ts   — 2블록 (duplicate x1, safe_ours x1)
  helpers.js — 1블록 (safe_theirs x1)
```
...
## Whole-Merge `-X` 전략 금지 (Commit-Level Strategy Pitfall)
